...
;if (SCAnum le 9999 and SCAnum gt 999) then SCAnumstr='0p'+string(SCAnum)
;SCAnumstr = STRREPLACE(STRTRIM(STRING(single_scattering, FORMAT='(F0.3)'),2), '.', 'p')
;SCAnumstr = STRTRAN(STRTRIM(STRING(single_scattering, FORMAT='(F0.3)'),2), '.', 'p')
SCAnum = ROUND(1000.0 * single_scattering)          ; 0..1000
s      = STRING(SCAnum, FORMAT='(I4.4)')            ; '0000'..'1000'
SCAnumstr = STRMID(s, 0, 1) + 'p' + STRMID(s, 1, 3) ; '0p234', '1p000'
print,single_scattering,SCAnum,SCAnumstr
JDnumstr=string(JD,format="(f15.7)")
  out_ideal_name=strcompress('OUTPUT/IDEAL/ideal_LunarImg_SCA_'+SCAnumstr+'_JD_'+JDnumstr+'.fit',/remove_all)
  writefits, out_ideal_name, image_I, header
...
free_lun,ijb
 ...
 ;writefits, strcompress('OUTPUT/SUNMASK/SunMask_LunarImg_'+string(iframe,format='(I4.4)')+'.fit',/remove_all), mask 
  writefits, strcompress('OUTPUT/SUNMASK/SunMask_LunarImg_'+string(IJD,format='(f15.7)')+'.fit',/remove_all), float(mask gt 0) 
...
; write_jpeg, strcompress('OUTPUT/JPEGS/MAPofEARTH_'+JDnumstr+'.jpg',/remove_all), bytscl(MAPofEARTH), quality=100
