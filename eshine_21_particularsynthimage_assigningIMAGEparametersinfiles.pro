...
; writefits, strcompress('OUTPUT/LunarImg_'+string(iframe,format='(I4.4)')+'.fit',/remove_all), image_I/max(image_I)*60000.0d0, hedr
  writefits, strcompress('OUTPUT/SUNMASK/SunMask_LunarImg_'+string(IJD,format='(f15.7)')+'.fit',/remove_all), mask
; writefits,strcompress('OUTPUT/LONLAT_AND_ANGLES_IMAGES/Angles_JD'+string(IJD,format='(f15.7)')+'.fits',/remove_all),theta_i_and_r_and_phi,header55
