...
def safe_topdir_glob(indir: Path, pattern: str) -> List[Path]:
    """
    Return only files directly inside indir (NO subfolders).

    - Rejects recursive or path-containing patterns.
    - Filters results so that p.parent == indir.
    """
    # Prevent accidental recursion or path traversal in the glob pattern
    if "**" in pattern or "/" in pattern or "\\" in pattern:
        raise ValueError(f"Refusing recursive/path glob pattern: {pattern!r}. Use e.g. '*.fit*'.")

    hits = []
    for p in indir.glob(pattern):  # non-recursive
        # Enforce top directory only
        if p.is_file() and p.parent.resolve() == indir.resolve():
            hits.append(p)
    return sorted(hits)
...
def main() -> int:
    ...
    ap.add_argument("--glob", type=str, default="*.fit*", help="Glob for FITS files (fits/fits.gz/fit). NO subdirs.")
    ...
    try:
        files = safe_topdir_glob(indir, args.glob)
    except ValueError as e:
        print(f"ERROR: {e}")
        return 2

    if not files:
        print(f"ERROR: no files matched {args.glob!r} in top directory {indir.resolve()} (no subfolders scanned)")
        return 2
    ...
    print(f"Scanning {len(files)} files in TOP dir only: {indir.resolve()}")
