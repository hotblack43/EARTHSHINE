...
;
; This version sets the viewing geometry and the lunar librations from an
; ephemeris. Also adds Poisson counting statistics, and other more or less
; realistic disturbances, to the CCD image
;
; Version 12
;
; Author: Hans Gleisner   (c) Danish Meteorological Institute
;
;===============================================================================
...
PRO eshine_core, JD, phase_angle, lat_lib, lon_lib, PA_lib, $
                 ...
                 image_I, image_16bit, image_info, image_number

common camera_information,exposure_time,camera_ID

;----------------
; Data depository.
;----------------
; datalib = 'C:\Documents and Settings\Peter Thejll\Desktop\ASTRO'
; datalib = 'C:\EarthShine\Simulations\Data_eshine'
; datalib = 'C:\Temp\Eshine\data_eshine'
datalib='/home/pth/moon/Eshine/data_eshine'
...
;------------------------------------------------------------------------
; Assign three image arrays: real-valued, 8-bit pixels, and 16-bit pixels.
;------------------------------------------------------------------------
pixelscale  = 2.5*CCDfactor         ; arc seconds per pixel (small factor => small pixels)
imsize      = CCDsize/CCDfactor     ; image size in terms of pixels (small factor => more pixels)
image       = dblarr(imsize,imsize) * 0.0d0
image_16bit = lonarr(imsize,imsize) * long(0)
...
;=     Discretize the real-valued intensity image to 16-bit pixels.            =
;=                                                                             =
;===============================================================================
...
; Create a new image array simulating a well-exposed 16-bit image.
;-----------------------------------------------------------------------
; scale the input image
maxval = 0.90*65535.0d0
image = maxval*image/max(image)        ; all values between 0 and maxval
image_I = image

; add degrading effects: Poisson noise + sky + CCD effects
...
if (if_poisson eq 1) then begin
  ...
  for j=0,nrows-1,1 do begin
    if (image[i,j] NE 0.0d) then image[i,j] = randomu(seed,poisson=image[i,j])
  endfor
  endfor
endif
sky = 0.0d
if (if_sky eq 1) then begin
; add counts for the sky - the Poisson noise will be generated by the call to generate_CCD
  skylevel  = 10.0d
  image=image+skylevel
endif
bias = 0.0d
RN   = 0.0d

if (if_CCDeffects eq 1) then begin
  generate_CCD,image,image_number,camera_ID,exposure_time,CCD_out,raw_CCD
...
;===============================================================================
;=                                                                             =
;=  7. Write some things to file.                                              =
;=     This stuff is better to implement in a separate analysis routine.       =
;=                                                                             =
;===============================================================================

; Yes, it has been removed. Analysis should be done externally from nowon.
