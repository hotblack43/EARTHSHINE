...
PRO eshine_15_GUI_event, event
  ...
  if strcmp(action,'quit',/fold) then begin
    ...
    imsize = fix(CCDsize)           ; image size in terms of pixels
    ...
    if_poisson    = 0
    if_CCDeffects = 0
    if (degrade_input[0] EQ 1) then if_sky = 1
    if (degrade_input[1] EQ 1) then if_poisson = 1
    ...
    ; 0=single image
      ...
      ipict = -1
      iframe = -1
      if (total(imgfileformats) GT 0.0) then begin
        ipict = 1
      endif
      ...
      eshine_imagetofile, image_16bit, image_info, camera_ID, exposure_time, imsize, imgfileformats, ipict, iframe, mpegID
    ...
    ; 1=image sequence
      ...
      ipict = -1
      iframe = -1
      if (seqfileformats[0] EQ 1 OR seqfileformats[1] EQ 1) then begin
        ipict = 0
      endif
...
PRO eshine_15_GUI
  ...
  base = WIDGET_BASE(title='DMI EarthShine Simulator v15',xoffset=5,yoffset=5,xsize=740,ysize=460)
  ...
  txtJDyr  = WIDGET_TEXT(base,/editable,value='2004',xoffset=157,yoffset=70,xsize=4,ysize=1)
  txtJDmon = WIDGET_TEXT(base,/editable,value=' 5',  xoffset=191,yoffset=70,xsize=2,ysize=1)
  txtJDday = WIDGET_TEXT(base,/editable,value='19',  xoffset=213,yoffset=70,xsize=2,ysize=1)
  txtJDhr  = WIDGET_TEXT(base,/editable,value='21',  xoffset=235,yoffset=70,xsize=2,ysize=1)
  ...
  drawSeparator = WIDGET_DRAW(base, xoffset=370, yoffset=25, xsize=1, ysize=340)

  ; Filters
  headerE1  = WIDGET_LABEL(base,value='FILTERS',font='arial*bold*16',xoffset=390,yoffset=30)
  btngrpFilter = CW_BGROUP(base,['half filter'],font='arial*16',set_value=0,row=1,/nonexclusive,xoffset=395,xsize=80,yoffset=48,ysize=30)

  ; Degrading effects
  headerDegrade = WIDGET_LABEL(base,value='DEGRADING EFFECTS',font='arial*bold*16',xoffset=390,yoffset=120)
  btngrpDegrade = CW_BGROUP(base,['Sky','Poisson','CCD'],font='arial*16',set_value=0,row=1,/nonexclusive,xoffset=395,xsize=200,yoffset=138,ysize=30)

  ; Options & Output -- action buttons
  headerRunMode = WIDGET_LABEL(base,value='RUN MODE & OUTPUT',font='arial*bold*16',xoffset=390,yoffset=210)
  btngrpAction  = CW_BGROUP(base,['image, single','image sequence'],font='arial*16',set_value=0,row=2,/exclusive,xoffset=395,xsize=120,yoffset=230,ysize=60)
  btngrpImgFile = CW_BGROUP(base,['FITS','TIFF','JPEG'],font='arial*16',set_value=[1,0,1],row=1,/nonexclusive,xoffset=530,xsize=200,yoffset=235,ysize=30)
  btngrpSeqFile = CW_BGROUP(base,['FITS','TIFF','MPEG'],font='arial*16',set_value=[0,0,1],row=1,/nonexclusive,xoffset=530,xsize=200,yoffset=268,ysize=30)
  lblSeqStep    = WIDGET_LABEL(base,value='step',xoffset=420,yoffset=300)
  lblSeqLength  = WIDGET_LABEL(base,value='length',xoffset=420,yoffset=325)
  lblSeqStepUnit   = WIDGET_LABEL(base,value='[hr]',xoffset=485,yoffset=300)
  lblSeqLengthUnit = WIDGET_LABEL(base,value='[days]',xoffset=485,yoffset=325)
  txtSeqStep    = WIDGET_TEXT(base,/editable,value='   ',xoffset=453,yoffset=295,xsize=3)
  txtSeqLength  = WIDGET_TEXT(base,/editable,value='   ',xoffset=453,yoffset=320,xsize=3)

  ; Start & Quit buttons
  btnRun  = WIDGET_BUTTON(base,value='Run',uvalue='run',font='arial*bold*italic*18',xsize=80,ysize=40,xoffset=440,yoffset=390)
  btnQuit = WIDGET_BUTTON(base,value='Quit',uvalue='quit',font='arial*bold*italic*18',xsize=80,ysize=40,xoffset=580,yoffset=390)

  ; Copyright label
  labelCopyright = WIDGET_LABEL(base,value='(c) Danish Meteorological Institute',font='arial*14',xoffset=5,yoffset=445)
