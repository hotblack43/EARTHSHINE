...
if strcmp(transform,'meeq2sel',/fold_case) then begin
  ...
  Rz[*,0] = [ costhetaZ ,-sinthetaZ ,   0.0D   ]
  Rz[*,1] = [ sinthetaZ , costhetaZ ,   0.0D   ]
  Rz[*,2] = [    0.0D     ,    0.0D     ,   1.0D   ]

  ; rotation around the new Y-axis (observe the rotation direction)
  costhetaY=cos(thetaY)
  sinthetaY=sin(thetaY)
  Ry[*,0] = [ costhetaY ,   0.0D   , sinthetaY ]
  Ry[*,1] = [    0.0D     ,   1.0D   ,     0.0D    ]
  Ry[*,2] = [-sinthetaY ,   0.0D   , costhetaY ]

  ; rotation around the new X-axis
  costhetaX= cos(thetaX)
  sinThetaX=sin(thetaX)
  Rx[*,0] = [   1.0D   ,    0.0D     ,   0.0D      ]
  Rx[*,1] = [   0.0D   , costhetaX ,-sinThetaX ]
  Rx[*,2] = [   0.0D   , sinThetaX , costhetaX ]
