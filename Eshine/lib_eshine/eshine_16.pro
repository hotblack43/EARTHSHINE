...
camera_ID = 'STL1001E'
;
CCDcols = 1025      ; image size in terms of pixels
CCDrows = 1025      ; image size in terms of pixels
pixelscale = 2.5    ; arc seconds per pixel


;-----------------------------------------------------------------------
; Filters
;-----------------------------------------------------------------------
if_filter  = 0            ; half-filter / no half-filter
...
imgfile = intarr(2)
filefmt = intarr(3)
imgfile[0] = 1       ; 'realistic' CCD image
imgfile[1] = 1       ; 'ideal' radiance image of the Moon
filefmt[0] = 1       ; FITS
filefmt[1] = 0       ; TIFF
filefmt[2] = 1       ; JPEG/MPEG


;-----------------------------------------------------------------------
; Make simulation, show on screen, and print to files
;-----------------------------------------------------------------------
iframe = 0
...
for iJD = JDstart,JDend,JDstep do begin
  ...
  writefits, strcompress('OUTPUT/LunarImg_ideal_'+string(iframe,format='(I4.4)')+'.fit',/remove_all), image_I, header
  ...
  write_jpeg, strcompress('OUTPUT/LunarImg_'+string(iframe,format='(I4.4)')+'.jpg',/remove_all), congrid(image_8bit,CCDcols,CCDrows), quality=80
