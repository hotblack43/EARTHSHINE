...
; then assembled image as would be read out
	...
	flat=gain*0.0d0
	for i=0,l(0)-1,1 do begin
	for j=0,l(1)-1,1 do if(gain(i,j) ne 0) then flat(i,j)=randomn(seed,poisson=gain(i,j)*full_well*0.5d0)
	endfor
; generate a suitable header
	MKHDR, header, flat
        sxaddpar, header, 'CCD_CAMERA','SIMULATED SXVH9','flat simulated using generate_ccd.pro'
	writefits,strcompress('OUTPUT/flat_'+string(image_number)+'.fit',/remove_all),flat,header
	...
	writefits,strcompress('OUTPUT/darkframe_'+string(image_number)+'.fit',/remove_all),darkframe,header
return
endif
;
...
; generate a suitable header
	MKHDR, header, flat
        sxaddpar, header, 'CCD_CAMERA','SIMULATED ST1001E','flat simulated using generate_ccd.pro'
	writefits,strcompress('OUTPUT/flat_'+string(image_number)+'.fit',/remove_all),flat,header
	MKHDR, header, darkframe
        sxaddpar, header, 'CCD_CAMERA','SIMULATED ST1001E','Simulated using generate_ccd.pro'
        sxaddpar, header, 'EXPTIME',string(exposure_time),'Sumulated exposure time'
	writefits,strcompress('OUTPUT/darkframe_'+string(image_number)+'.fit',/remove_all),darkframe,header
return
endif
