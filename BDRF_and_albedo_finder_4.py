...
def update_brdf(all_valid_angles, all_reflectance_values):
    ...
    try:
        ...
        print(f"⚠️ LinearNDInterpolator failed: {e}. Switching to NearestNDInterpolator.")
...
def iterative_solve(fits_directory, output_fits_path, angle_units="degrees", max_iterations=10):
    ...
    Learns BRDF collectively from all images.
    """
    ...
    # Initialize albedo as a running average
    albedo_map = None
    ...
    for idx, fits_cube_path in enumerate(fits_files):
        ...
        # Ensure brdf_func is initialized
        if idx == 0:
            brdf_func = lambda x: 1.0  # Default function returning constant reflectance
        ...
        for iteration in range(max_iterations):
            ...
            for i in range(incidence_map.shape[0]):
                for j in range(incidence_map.shape[1]):
                    if valid_mask[i, j]:  # Only solve for valid pixels
                        albedo_map[i, j] = update_albedo(
                            incidence_map[i, j], emergence_map[i, j], phase_angle, radiance_map[i, j], brdf_func
                        )

            # Step 7B: Collect valid data for BRDF update
            ...
            reflectance_values = radiance_map.flatten()[valid_pixels] / albedo_map.flatten()[valid_pixels]
        ...
        # Step 7C: Update BRDF using all collected data
        if num_images_processed > 1:
            print("  🔄 Updating BRDF using all processed images...")
        brdf_func = update_brdf(all_valid_angles, all_reflectance_values)

    # Compute final albedo map as a smoothed running average
    final_albedo_map = np.mean(albedo_map, axis=0)
