#!/usr/bin/env python3
#
# uv run  glue_cubes_4.py --layer-desc-file layer_descriptions.txt --no-obs-syn-keys
#
...
def write_combined(
    ...
    # Remove any pre-existing layer labelling keys inherited from the observational header.
    # Otherwise you can end up with BOTH LAYER0/LAYER1/... and LAYER01/LAYER02/... etc.
    ...
    # Write combined list as LAYER01..LAYERnn (no comment text; saves space and avoids confusion)
    for i, nm in enumerate(all_names, start=1):
        hdr[f"LAYER{i:02d}"] = (nm, "")

    # Optionally write separate groupings (duplicates of LAYER##, but sometimes convenient)
    if not no_obs_syn_keys:
...
def main() -> int:
    ...
    ap.add_argument(
        "--no-obs-syn-keys",
        action="store_true",
        help="Do not write OBS## and SYN## header keys (only write LAYER##).",
    )
    ...
    no_obs_syn_keys = bool(args.no_obs_syn_keys)
...
#           out_path = out_dir / f"combined_JD{jd}_{obs_path.stem}.fits"
            out_path = out_dir / f"MLOcube_{obs_path.stem}.fits"
            write_combined(
                out_path,
                obs_cube,
                synth_cube,
                ...
                user_layer_desc=user_layer_desc,
                user_layer_desc_file=user_layer_desc_file,
                no_obs_syn_keys=no_obs_syn_keys,
            )
