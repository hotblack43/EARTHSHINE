...
;------------------------------------------
 ...
 parinfo[*].value = p
 ;
 print,'These parameters are fixed: ',parinfo(*).fixed
 w1=30
 w2=70
; set up the multipatch pointers
print,'About to use yoffset 2: ',yoffset
 ...
 PARINFO=parinfo, $
 PERROR=sigs,niter=niter,covar=covariance,/nan)
;To compute the correlation matrix, PCOR, use this example:
 ...
 close,38
 openw,38,'covariance_matrix.dat'
 printf,38,format='('+string(n_elements(parms))+'(1x,g8.2))',covariance
 close,38
 erralbedo=sigs(3)
 return
 end
 ...
 while not eof(3) do begin
     ...
     readf,3,name
         ...
         print,'JD: ',jd
	 mphase,jd,illfrac
     if (name eq 'STOP' or name eq 'stop' or illfrac gt 0.5) then stop
     file=lowpath+name
 ...
 rawnrows=lll(1)
     if (x0-radius gt 31 and rawncols-1-(x0+radius) gt 31 ) then begin
         ...
         nbands=5
             ...
             openw,63,'CLEM.profiles_fitted_results_SELECTED_5_multipatch_100_smoo_SINGLES.txt',/append
