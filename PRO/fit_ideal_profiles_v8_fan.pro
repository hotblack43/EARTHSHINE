 ...
 PRO use_cusp_angle_build_fan,im,x0,y0,radius,rad,line,err_line,imethod
 ...
 ;....
 r_step=4
 w=30.	; half-width of fan in degrees degrees
 if (ipntr eq  1) then idx=where(theta ge cangle(0)-w and theta lt cangle(0)+w and xline gt x0)
 if (ipntr eq -1) then idx=where(theta ge cangle(0)-w and theta lt cangle(0)+w and xline lt x0)
 ;
 ;
 rad=dblarr(512)*0-911
 line=dblarr(512)*0-911
 err_line=dblarr(512)*0-911
 ic=0
 for r=10,501-r_step,r_step do begin
     markedupimage=im
     jdx=where(radii(idx) gt r and radii(idx) le r+r_step)
     if (jdx(0) ne -1) then begin
         rad(ic)=median(radii(idx(jdx)))
         err_line(ic)=stddev(im(idx(jdx)))
...
;        print,ic,rad(ic),line(ic),err_line(ic),n_elements(idx(jdx))
     ic=ic+1
 markedupimage(idx(jdx))=max(markedupimage)
 tvscl,markedupimage
     endfor
 kdx=where(rad ne -911)
 rad=rad(kdx)
 line=line(kdx)
 err_line=err_line(kdx)
 return
 end
 ...
 common stuff117,name,use1,use2,im1,im2,raw,RMSE,x0,y0,radius,nbands,w1,w2
 common namingstuff,JD,labelstr,markedupimage,if_smoo,if_jiggle,jiggle_ampl
 ...
 use_cusp_angle_build_fan,folded,x0,y0,radius,rad_folded,line_folded,dummy,imethod
 ...
 line=Yobs
 resids=(line-value)
 RMSE=sqrt(total(resids^2)/n_elements(line))
 ;---------legend
 ystart=!Y.crange(1)
 ...
 xyouts,q,ystart-7.*ystep,'contrast   = '+string(contrast,format='(f7.4)')
 ...
 WRITE_JPEG,strcompress('OUTPUT/'+labelstr+string(JD,format='(f15.7)')+'.jpg',/remove_all),tvrd(true=1),true=1
 ;---------------------
 return,value
 END
 
 PRO gofitFAN,albedo,erralbedo,alfa1,rlimit,pedestal,xshift,yshift,corefactor,contrast
 ...
 contrast=0.8
 yshift=0.0;47131281
 p = [alfa1,rlimit,pedestal,albedo,xshift,corefactor,contrast,yshift,zodi,SLcounts]
 ...
 parinfo[*].value = p
 ;
 w1=30	; left of disc edge on sky
 w2=70	; right of disc edge on disc
 ; get the 'fan' for observations
 kmethod=3       ; i.e. mean halfmedian of annulus-segments
 use_cusp_angle_build_fan,raw,x0(0),y0(0),radius,rad_raw,line_raw,erry,kmethod
 y=line_raw
 ...
 niter=500
 parms = MPFITFUN('foldnFAN', X, Y, erry, p, yfit=yfit, $
 PARINFO=parinfo, $
 ...
 ;........................................................
 ; Version 8 of code that finds albedo by model-fitting
 ; uses cusp-angle 'fan' to fit on
 common stuff117,name,use1,use2,im1,im2,raw,RMSE,x0,y0,radius,nbands,w1,w2
 common namingstuff,JD,labelstr,markedupimage,if_smoo,if_jiggle,jiggle_ampl
 common fanstuff,rad_raw,line_raw,rad_folded,line_folded
 common headerstuff,mixedimageheader
 ...
 while not eof(3) do begin
     ...
     if (illfrac lt 0.8) then begin
         ...
         if (x0-radius gt 31 and rawncols-1-(x0+radius) gt 31 ) then begin
             ...
             im2=im2_good
             ;..........................................
             gofitFAN,albedo,erralbedo,alfa1,rlimit,pedestal,xshift,yshift,corefactor,contrast
             print,'RMSE: ',RMSE
             print,'Albedo:',albedo,' +/- ',erralbedo
             totfl=total(raw-pedestal,/double)
             fmtstr44='(f15.7,9(1x,f9.5),1x,f10.3,1x,f15.3,2(1x,f10.7),2(1x,a))'
             openw,63,'CLEM.profiles_fitted_results_fan_TEST.txt',/append
             printf,63,format=fmtstr44,JD,albedo,erralbedo,alfa1,rlimit,pedestal,xshift,yshift,corefactor,contrast,RMSE,totfl,zodi,SLcounts,name,labelstr
             close,63
