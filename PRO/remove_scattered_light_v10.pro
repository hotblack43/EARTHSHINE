...
if (STRUPCASE(typeflag) eq "CIE") then begin
		...
		readf,83,height,scale	; CIE profile case
...
if (STRUPCASE(typeflag) eq "CIE") then P=[height,scale]	; CIE profile case
...
print,'RMSE=',err,' scale=',scale,' factor=',factor
...
; This version applies a CIE profile to an image without bias, and removes it
;---------------------------------------------------------------------------------
common moonres,im1,im2,im3
common uselater,im4,difference
common type,typeflag
