...
PRO go_contrast_im2,im2,im2contrasted,contrast
 ...
 ; now add the ZL correction to the sky-only pixels
 folded(idx)=folded(idx)+zodi
 ; and add the starlight correction to the same background
 folded(idx)=folded(idx)+SLcounts
 tvscl,folded
;------------------------------------------
 w1=30
 w2=70
...
;------------------------------------------
 ...
 xyouts,10,ystart-10.*ystep,'ZL = '+string(zodi,format='(f7.4)')
 xyouts,10,ystart-11.*ystep,'SL = '+string(SLcounts,format='(f7.4)')
...
;To compute the correlation matrix, PCOR, use this example:
 ...
 print,format='(9(1x,g8.2))',covariance
 ...
 rawnrows=lll(1)
     if (x0-radius gt 31 and rawncols-1-(x0+radius) gt 31 ) then begin
         ...
         nbands=5
             ...
             openw,63,'CLEM.profiles_fitted_results_multipatch_TRIALIMAGES_ZODI_STARL.txt',/append
