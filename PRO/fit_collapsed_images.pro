PRO go_contrast_im2,im2,im2contrasted,contrast
 ...
 FUNCTION foldnstuff_collapse, X, P
 ...
 common namingstuff,JD,bandstr
 common switches,if_want_collapsed
 alfa1=p(0)
 rlimit=p(1)
 pedestal=p(2)
 albedo=p(3)
 ...
 set_plot,'X'
 !P.MULTI=[0,1,1]
 if (if_want_collapsed ne 1) then begin
...
; i.e. we want to collapse the trial image
	value=avg(folded,1,/double)
	value=value(use1:use2)
        ladder_plot,x,avg(raw,1,/double),value,'Columns','Observed and fitted counts',bandstr+name
	line=avg(raw,1,/double)
 endelse
 print,'Back from ladder plot'
 y=line(use1:use2)*1.0d0
 resids=(y-value)
 RMSE=sqrt(total((resids)^2)/n_elements(y))
 res12=linfit(findgen(n_elements(resids)),resids,yfit=yhat)
 print,'SD on residuals on DS sky: ',stddev(resids(10:40)-yhat)
 ;---------legend
 ystart=!Y.crange(1)
 ...
 line=avg(raw(*,row_from:row_to),1)
 y=line(use1:use2)*1.0d0
 erry=sqrt(abs(y))/sqrt(21.*100.)	; typically 21 rows in 100 images make up each 'band'
 if (if_want_collapsed eq 1) then begin
 	y=avg(raw,1,/double)
 	y=y(use1:use2)*1.0d0
 	erry=y*0.0d0+0.015	; guesswork!
 endif
 maxiter=500
 parms = MPFITFUN('foldnstuff_collapse', X, Y, erry, p, yfit=yfit, $
 PARINFO=parinfo,status=hej, $
 PERROR=sigs,niter=niter,maxiter=maxiter)
 alfa1=parms(0)
 rlimit=parms(1)
 pedestal=parms(2)
 albedo=parms(3)
...
print,'Status was: ',hej
print,'# of iterations performed was: ',niter
stop
 erralbedo=sigs(3)
 return
 end
 ...
 nbands=4
 if_want_collapsed=1
 ;if (if_want_collapsed eq 1) then nbands=0
 ...
 while not eof(3) do begin
     ...
     if (x0-radius gt 40 and 511-(x0+radius) gt 40) then begin
         ...
         for iband=-nbands,nbands,1 do begin
             bandstr=bandnames(nbands+iband)
             im1=im1_good
             im2=im2_good
             ;..........................................
             ...
             fmtstr44='(f15.7,8(1x,f8.4),1x,f10.3,1x,f15.3,1x,a,1x,a)'
             openw,63,'CLEM.profiles_fitted_results_April_2014_morebands.txt',/append
             printf,63,format=fmtstr44,JD,albedo,erralbedo,alfa1,rlimit,pedestal,xshift,corefactor,contrast,RMSE,totfl,name,bandstr
             close,63
             endfor
         endif
