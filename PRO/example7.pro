; code like example4.pro, which compars SAL to TRS/TIS but in example7.pro we also
; try to compensate for the mean(cos(sza)) effect by dividing TRS by that quantity using precalculated
; fields for each montha nd year. These are binary files calculated by "calculate_mean_cossza.pro",
; and the files are found in "/cmasf/cmsaf-cld3/pthejll/"
;=================================
...
get_cossza_file,meanCosSZAgrid,londummy,latdummy,strmid(CFCfile,45,6)
...
; correct the SAL from nominal 60 degrees SZA to whatever it really is, which is given in 
; the array meanCosSZAgrid
...
scatter_col,SALgridded(idx),albedo(idx),minx=0.,miny=0.,maxx=1.0,maxy=1.0,binx=0.01,biny=0.01,color_bar=color_bar,ytitle='TRS/TIS',xtitle='SAL_60*d_function',title=strcompress(tstr)
oplot,[0,1],[0,1]
endif
endfor
close,1
