...
PRO gostripthename,str,basename
 ...
 printf,82,format='(f15.7,3(1x,f8.2))',jd,x0,y0,radius
 return
 end
...
;        window,1,xsize=400,ysize=300
         plot,yrange=[0,max(y(idx))],x(idx),y(idx),psym=7,ystyle=1,title='Angle='+string(theta(0)),xtitle='Distance from Moon ctr.'
 ...
 PRO remove_scattered_light_lin_or_log_method,iflog,observed_image,clean_image,inside,outside,DSonleft
...
;............................
 ...
 for theta=180.0d0,360.0d0-theta_step,theta_step do begin
     ...
     for ibin=0,nbins-2,1 do begin
         idx=where(radii ge radbins(ibin) and radii lt radbins(ibin+1) and angle ge theta and angle lt theta+theta_step)
         if (idx(0) ne -1) then begin
             if (n_elements(idx) ge 4) then printf,44,ibin,theta,median(radii(idx)),median(observed_image(idx)),stddev(observed_image(idx))/sqrt(n_elements(idx))*fudge
             ;if (n_elements(idx) ge 4) then printf,44,ibin,theta,mean(radii(idx)),mean(observed_image(idx)),stddev(observed_image(idx))/sqrt(n_elements(idx))*fudge
;            if (viz eq 1) then begin
;                window,0
;                im=observed_image
...
;............................
 ...
 PRO get_observed_image,inname,observed_image,stack,header,cg_x,cg_y
 common circleSTUFF,circle,radius,moon_coords
 common vizualise,viz
 common logarithmic,iflog
 stack=readfits(inname,header)
 observed_image=stack(*,*,0)
 l=size(observed_image,/dimensions)
...
;if (viz eq 1) then tvscl,alog(imin2)
 ...
 ; Version 5: Works by setting up a loop on the files to be treated, and knwos which 
 ; side of the image to reduce - i.e. it knows where the DS is.
 ;.....................................................................................
 close,/all
 common paths,path
 common circleSTUFF,circle,radius,moon_coords
 ...
 common vizualise,viz
 viz=1
 iflog=0	; 0 is usual linear images, 1 is take logs
 common lineandpoint,line,point1
 common which,whichcleaned
 common date,jd
 openw,82,'detectedradiiandcentres.dat',/append
 outpath='/data/pth/UNIVERSALSETOFMODELS/newH-63_HIRESscaled_LA/convolved_albedo0p31/BBSO_CLEANED_IMAGES/'
;spawn,'rm '+outpath+'* '
 line=[256,0,256,511]
 point1=[511,0]
 imstring='/data/pth/UNIVERSALSETOFMODELS/newH-63_HIRESscaled_LA/convolved_albedo0p31/'
;files=file_search(imstring+'LunarImg_*.fit',count=n)
 files=file_search(imstring+'*.fits',count=n)
 print,'Found ',n,' files to work on, i n directory ',imstring
 for i=0,n-1,1 do begin
     get_JD_from_filename,files(i),JD
	mphase,jd,k
if (k le 0.5) then begin
     gostripthename,files(i),basename
     observed_image_name=files(i)
     get_observed_image,observed_image_name,observed_image,stack,header,cg_x,cg_y
...
;	klo=get_kbrd()
     remove_scattered_light_lin_or_log_method,iflog,observed_image,cleaned_image,inside,outside,DSonleft
     ;
     if (iflog eq 0) then outname=strcompress(outpath+'BBSO_CLEAN_'+basename,/remove_all)
     if (iflog eq 1) then outname=strcompress(outpath+'BBSO_CLEAN_LOG10_'+basename,/remove_all)
     if (iflog eq 0) then writefits,outname,[[[stack]],[[cleaned_image]]],header
     if (iflog eq 1) then writefits,outname,[[[stack]],[[10^cleaned_image]]],header
     print,'Wrote ',outname
