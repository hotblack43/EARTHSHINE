 PRO get_all_ideal_models,jd_in,image_stack,labels,wanted_type_of_model_file
 jd=string(jd_in,format='(f15.7)')
 path='/media/thejll/OLDHD/UNIVERSALSETOFMODELS/*'
 path='/data/pth/UNIVERSALSETOFMODELS/*'
 pathlen=strlen(path)
 name=strcompress('*'+jd+'*.fit*',/remove_all)
 files=file_search(path,name,count=n)
 idx=where(strpos(files,wanted_type_of_model_file) ne -1)
 files=files(idx)
 n=n_elements(files)
 image_stack=[]
 labels=[]
...
PRO    gettheflatfield,filter,flatfield
p='/home/pth/SCI/EARTHSHINE/'
if (filter eq 'B') then flatfield=readfits(p+'B_superflat.fits')
if (filter eq 'V') then flatfield=readfits(p+'V_superflat.fits')
if (filter eq 'VE1') then flatfield=readfits(p+'VE1_superflat.fits')
if (filter eq 'VE2') then flatfield=readfits(p+'VE2_superflat.fits')
if (filter eq 'IRCUT') then flatfield=readfits(p+'IRCUT_superflat.fits')
return
end
...
stop
 ...
 xline=intarr(l)
 yline=intarr(l)
 print,'In use_cusp_angle_fan_DS_BS, x0,y0,radius:',x0,y0,radius
 for icol=0,l(0)-1,1 do begin
     for irow=0,l(1)-1,1 do begin
         xline(icol,irow)=icol
         yline(icol,irow)=irow
...
; BS fan
 ...
 if (if_viz eq 1) then tvscl,markedupimage
...
if (w3 ne w4) then begin
 ...
 err_line=reform(data(2,*)) 
	...
	stop
	endelse
 if (if_viz eq 1) then ploterr,rad,line,err_line,psym=7
 return
 end
...
possible_offset=-mean(raw(0:10,0:10))
print,'The possible pedestal is: ',possible_offset
print,'p(2) is: ',p(2)
;a=get_kbrd()
 ...
 print,'aboutt omix images, albedo=',albedo
 ...
 print,'before Zodiacal, so many =0: ',n_elements(idx)
 ; fold 
...
;..............
 if (if_viz eq 1) then tvscl,folded
 writefits,strcompress('OUTPUT/IDEAL/synth_folded_scaled_shifted_JD'+string(JD,format='(f15.7)')+'.fits',/remove_all),folded,header
 ...
 set_plot,'X'
 !P.MULTI=[0,1,1]
 if (if_viz eq 1) then ladder_plot,x,Yobs,value,'Columns','Observed and fitted counts',labelstr+name
 line=Yobs
 resids=(line-value)
 w=1./erry
 ...
 PRO gofitFAN,albedo,erralbedo,alfa1,bwpr,pedestal,xshift,yshift,acoeff,lamda0
 ...
 common erry,erry
 common fremoveZLandSL,ifremoveZLandSL,exptime
...
print,'hejsa'
 ...
 print,'Solution:'
 if (sigs ne !NULL) then begin
 print,'alfa1       :',alfa1,' +/- ',sigs(0)
 print,'bwpr      :',bwpr,' +/- ',sigs(1)
...
print,'     alfa   bwpr  pedestal  albedo   xshift   corefac lamda0  yshifta     ZL       SL'
 ...
 close,38
 erralbedo=sigs(3)
 if (erralbedo eq !NULL) then stop
 endif else begin
	print,'sigs are undefined ...'
 endelse
 return
 end
 ...
 ; fetch two precalculated ideal FITS images of the Moon for the given JD with Earth albedo 0 and 1
 get_lun,hjkl
 ...
 free_lun,hjkl
 ;...get the image
 spawn,'idl go_get_particular_synthimage_21.pro'
 ...
 ;........................................................................................
 close,/all
 ; Version 100 of code that finds albedo by model-fitting
 ; Uses a many-term polynomial PSF
 ; Uses large weighting on DS sky, low weighting of edge.
 ; fits 6 models to each observed image
 ; Models are taken from a list of precalæculated models
 ;........................................................................................
 common stuff117,name,use1,use2,im1,im2,raw,RMSE,x0,y0,radius,nbands,w1,w2,w3,w4
 common namingstuff,JD,labelstr,markedupimage,if_smoo,if_jiggle,jiggle_ampl
 common fanstuff,rad_raw,line_raw,rad_folded,line_folded
 ...
 common thetaflags,iflag_theta,radii,theta,xline,yline
 common backgroundcounts,zodi,SLcounts
 common zodiacal,zoddata,delta_lon,delta_lat
 common fremoveZLandSL,ifremoveZLandSL,exptime
 common eclipticcorrdsforMoon,jdCOMMON,raCOMMON,decCOMMON,moonECLlonCOMMON,moonECLlatCOMMON
 common filterstuff,filter,if_want_replacedBS
 common cuspanglestuff,iflagcuspangle,cangle
;common sukminkwoon,delta_lonSMK,delta_latSMK,zoddataSMK
 common sukminkwoon,iflagSMK,delta_lonSMK,delta_latSMK,zoddataSMK

 common vizualize,if_viz


 wantedmodeltypeslist=['H-X_HIRESscaled_LA', 'H-X_HIRESscaled_LS', 'H-X_LRO_LA', 'H-X_LRO_LS', 'newH-63_HIRESscaled_LA', 'newH-63_HIRESscaled_LS', 'newH-63_LRO_LA', 'newH-63_LRO_LS']
 ifremoveZLandSL=1
 iflagSMK=1
 ...
 !P.CHARSIZE=1.4
;.....................
 openr,3,'testimages.txt'
 while not eof(3) do begin
     iflag_theta=1
     item=''
     readf,3,item
     ;bits=strsplit(item,'/',/extract)
     name=item ; bits(4)
     bits2=strsplit(name,'_',/extract)
     filter=bits2(1)
     ...
     if (illfrac lt 0.6) then begin
	     ...
	     ; first get ALL ideal images for this JD
             for typewanted=0,8-1,1 do begin
             wanted_type_of_model_file= wantedmodeltypeslist(typewanted)
             get_all_ideal_models,JD,image_stack,labels,wanted_type_of_model_file
             nidealstack=n_elements(labels)
             for istacklooper=0,nidealstack-1,2 do begin
             ;.......get the two synthetic ideal images
             im1_good=image_stack(*,*,istacklooper)
             im2_good=image_stack(*,*,istacklooper+1)
;            get_two_synthetic_images,JD,im1_good,im2_good,mixedimageheader
             labelstr=strmid(labels(istacklooper),0,strlen(labels(istacklooper))-4)
             im1=im1_good
             im2=im2_good
             ;..........................................
             gofitFAN,albedo,erralbedo,alfa1,bwpr,pedestal,xshift,yshift,acoeff,lamda0
             print,'RMSE: ',RMSE
             if (erralbedo ne !NULL) then print,'Albedo:',albedo,' +/- ',erralbedo
             totfl=total(raw-pedestal,/double)
             fmtstr44='(f15.7,9(1x,f9.5),1x,f10.3,1x,f15.3,2(1x,f10.7),1x,g15.6,2(1x,a),1x,a)'
             openw,63,'CLEM_justtesting.txt',/append
             flux=totfl/exptime
             if (erralbedo ne !NULL) then printf,63,format=fmtstr44,JD,albedo,erralbedo,alfa1,bwpr,pedestal,xshift,yshift,acoeff,lamda0,RMSE,totfl,zodi,SLcounts,flux,name,labelstr,filter
             if (erralbedo eq !NULL) then printf,63,format=fmtstr44,JD,albedo,-1.0,alfa1,bwpr,pedestal,xshift,yshift,acoeff,lamda0,RMSE,totfl,zodi,SLcounts,flux,name,labelstr,filter
             close,63
             writefits,'markedupimage.fits',markedupimage,header
             endfor	; end istacklooper loop
	 endfor	; end of wantedtype loop
             endif else begin
