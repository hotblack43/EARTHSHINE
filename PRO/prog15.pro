PRO generate_x_and_y,icase,n,noiseAC1,a,b,eta,target,proxy,loctemp,noise
 ...
 ; also does FITEXY as one of the regression methods
 ; also does first differencing on data if wanted before submitting to all the tests
 ;---------------------------------------------------------------------
 openr,25,'experiment.name'
 name_str='_17'	; a string with some sort of name for various files
 readf,25,name_str
 close,25
 ...
 test_type=2	; choose whether to test on all contiguous data or mirror-symmetric or
                ; a fraction of contiguous data (i.e. near training interval)
 if_firstdiff=1 ; whether to make first differences of data or not
 ...
 icase=1	; 1: add noise to proxy, 2: add noise to target, 3: to both
 if (icase eq 1) then begin
