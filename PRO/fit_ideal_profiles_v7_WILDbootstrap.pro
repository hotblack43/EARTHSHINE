PRO use_cusp_angle_build_fan,im,x0,y0,radius,rad,line,imethod
 ...
 common Y, Yobs
 common fanstuff,rad_raw,line_raw,rad_folded,line_folded,combineBANDSmethod
 common yoffset,yoffset
 common residualstuff,residualimage,folded
 ...
 for iband=-nbands,nbands,1 do begin
     ...
     if (iband gt -nbands and combineBANDSmethod eq 1) then value=[value,value_band]
     if (iband gt -nbands and combineBANDSmethod eq 2) then value=value+value_band
     endfor
 writefits,'markedupimage.fits',markedupimage,header
 if (combineBANDSmethod eq 2) then value=value/float(nbands*2+1)
 ...
 if (if_jiggle eq 1) then begin
     p=p*(1.0+jiggle_ampl/100.0*randomn(seed,n_elements(p)))
     p(1)=3.0
     p(5)=35.0
     endif
 ...
 for iband=-nbands,nbands,1 do begin
     ...
     if (iband gt -nbands and combineBANDSmethod eq 1) then y=[y,yband]
     if (iband gt -nbands and combineBANDSmethod eq 2) then y=y+yband
     endfor
 if (combineBANDSmethod eq 2) then y=y/float(nbands*2+1)
 ...
 ;........................................................
 ; Version 7 of code that finds albedo by model-fitting
 ; after first fit it will perform WILD BOOTSTRAPPING nWILD times
 common stuff117,name,use1,use2,im1,im2,raw,RMSE,x0,y0,radius,nbands
 common namingstuff,JD,labelstr,markedupimage,if_smoo,if_jiggle,jiggle_ampl
 common fanstuff,rad_raw,line_raw,rad_folded,line_folded,combineBANDSmethod
 common headerstuff,mixedimageheader
 ...
 if_smoo=1
 if_jiggle=0
 jiggle_ampl=0.0	;jiggle starting guesses by this many percent	%
 combineBANDSmethod=2
 nWILD=100
 !P.MULTI=[0,1,1]
 !P.thick=2
 !P.CHARSIZE=1.4
 ...
 while not eof(3) do begin
     ...
     rawncols=lll(0)
     rawnrows=lll(1)
     if (x0-radius gt 47 and rawncols-1-(x0+radius) gt 47 ) then begin
         ...
         writefits,'residualsimage.fits',residualimage
         writefits,'PCTrelativeresiduals.fits',(residualimage/folded)*100.
         print,'RMSE: ',RMSE
         ...
         print,'Not glad about your x0,y0: ',x0-radius,rawncols-1-(x0+radius),' lt 47?'
         endelse
