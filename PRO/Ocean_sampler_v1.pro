...
FUNCTION parameter_update,a,a_scale
; takes a random walk step
a=a+randomn(seed)*a_scale
;a=abs(a)
return,a
end
...
start_parms=[60.639172,  4.8747334e-07,  0.00029165780 ] ; these are h,lamda/(rho*Cp),kappa starting guesses
 ...
 ag=start_parms(0) & ag_scale=60./15.
 bg=start_parms(1) & bg_scale=4e-7/15.
 cg=start_parms(2) & cg_scale=.0002/15.
 ...
 for iloop=0L,nloop-1,1 do begin
     ...
     agupdated=parameter_update(ag,ag_scale)
     ; calculate prob of updated model fitted to data (p1)
     p1=likelihood(agupdated,bg,cg)
...
; then
     ...
     bgupdated=parameter_update(bg,bg_scale)
     ; calculate prob of updated model fitted to data (p1)
     p1=likelihood(ag,bgupdated,cg)
     ; let alpha=min[1,p1/p2]
     alpha2 = min([1,p1/p2])
     ; draw u
     u=randomu(seed)
...
; and finally
     ...
     p2=likelihood(ag,bg,cg)
     ; generate a random-walk new guess for the other parameter
     cgupdated=parameter_update(cg,cg_scale)
     ; calculate prob of updated model fitted to data (p1)
     p1=likelihood(ag,bg,cgupdated)
     ; let alpha=min[1,p1/p2]
     alpha2 = min([1,p1/p2])
     ; draw u
     u=randomu(seed)
