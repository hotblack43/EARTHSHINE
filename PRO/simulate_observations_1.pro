PRO preprocess_image,im_in,im_out
; Purpose is to scale any object so that the 
; brightest pixel is 90% of a 16 bit range
scalingfactor=0.9*(2L^16L-1L)
im_out=im_in/max(im_in)*scalingfactor
return
end
...
nBIG=10000
table=fltarr(5,nBIG)
file='/home/pth/moon/libration.dat'
openr,68,file
line=''
astr=''
l=0.0
...
flat=flat/float(nrows*ncols)*slope+1.	; a 1% slope
...
PRO generate_CCD,object_in,camera_ID,skylevel,exposure_time,CCD_out,raw_CCD
;===================================================================================
; Routine to simulate realistic CCD images
; Version 1 - omits scattered light in optical system
;----------------------------------------------------------------------------------
;  object_in  (INPUT)   : The 'scene' as it enters the telescope - i.e. this
;                         is the object LESS any scattered light due to the
;                         atmosphere. NOTE: object_in must be scaled to realistic counts
;                         (e.g. max < 2^16 -1; min >= 0) but may be a float or double.
;                         object_in must also contain any object Poisson noise.
;  camera_ID  (INPUT)   : A string identifying the camera characteristics
;  skylevel   (INPUT)   : The mean count per pixel due to isotropic sky background
;  exposure_time (INPUT): The exposure time in seconds - sets dark current etc.
;
;  CCD_out    (OUTPUT)  : The simulated CCD image - LONG integer
;  raw_CCD   (OUTPUT)  : The simulated CCD image - double float
;----------------------------------------------------------------------------------
;
; Basic equation used is: CCD = (Object+Sky)*Flat+Bias+Dark+RN, where
; Sky = photons from the sky
; Flat= flat field frame, i.e. the 'gain' of the CCD - a map of pixel sensitivities
; Bias= offset added by camnera electronics - fixed pattern map
...
if (strupcase(camera_ID) eq strupcase('simulate_perfect_CCD')) then begin
	...
	raw_CCD=(object_in+Sky)*Flat+Bias+RN
	CCD_out=LONG(raw_CCD)
	help,object_in,Sky,Flat,Bias,RN
	return
endif
if (strupcase(camera_ID) eq strupcase('SXVH9')) then begin
	Sky=object_in*0.0d0+skylevel
...
PRO eshine_core, JD, moon_albedo, moon_BRDF, earth_albedo, earth_BRDF, $
                 filter, filterfact, CCDimage_long,CCDimage_double
...
eq2hor, RAmoon, DECmoon, JD, moon_altitude, az, ha, LAT=observatory_latitude , LON=observatory_longitude , OBSNAME='mso' , $
...
; 1=albedo climatology from file
  ...
  ALBearth = fltarr(360,180) + 0.3+0.03*sin(JD/1.0d0*2.0*!pi)
...
for iz=-imsize/2,imsize/2 do begin
  ...
  if hitMoon then begin
    ...
    if (phi EQ 0.0D) then begin
      ...
      tanphi=tan(phi)
      B = 2.0 - (tanphi/(2*g)) * (1.0 - exp(-1.0*g/tanphi)) * (3.0 - exp(-1.0*g/tanphi))
...
;    S = (2.0/(3*!DPI)) * (sin(abs(phi)) + (!DPI-abs(phi)) * cos(abs(phi)))/!DPI + t*(1.0 - 0.5*cos(abs(phi)))^2
    cosabsphi=cos(abs(phi))
    S = (2.0/(3*!DPI)) * (sin(abs(phi)) + (!DPI-abs(phi)) * cosabsphi)/!DPI + t*(1.0 - 0.5*cosabsphi)^2
...
;--------------------------------------------
imsaved=image
mphase,jd,k ; get the illuminated fraction of the Moon, if less than 50% filter the rhs, if more, the lhs
split=0.3
if (filter GE 1 and phase_angle_E le 180.0) then begin
  for iy=imsize*split,imsize-1 do image[iy,*] = image[iy,*]/float(filterfact)
endif
if (filter GE 1 and phase_angle_E gt 180.0) then begin
  for iy=0,imsize*split-1 do image[iy,*] = image[iy,*]/float(filterfact)
endif
;
camera_ID='sxvh9'
;camera_ID='simulate_perfect_CCD'
skylevel=10.0d0
exposure_time=3.0d0
preprocess_image,image,image_to_turn_into_CCD	; i.e. scale the image to max nearly=65000
generate_CCD,image_to_turn_into_CCD,camera_ID,skylevel,exposure_time,CCDimage,CCDimage_double
...
CCDimage_long=CCDimage

;--------------
; Analyze image
;--------------
hist = histogram(CCDimage_long)
...
;------------
blockwidth=1
patch1=CCDimage_long(x1-blockwidth:x1+blockwidth,y1-blockwidth:y1+blockwidth)
patch2=CCDimage_long(x2-blockwidth:x2+blockwidth,y2-blockwidth:y2+blockwidth)
counts1=mean(patch1)
counts2=mean(patch2)
std1=stddev(patch1)
std2=stddev(patch2)
illum_ratio=counts1/counts2
illum_ratio_err=sqrt((std1/counts1)^2+(std2/counts2)^2)*illum_ratio
fmt='(7(1x,f11.4),1x,f10.6)'
caldat,jd,mm,dd,yy,hh,mm,ss
doy=double(jd)-double(julday(1,1,yy))

if (if_moon_visible eq 1) then begin
    openw,33,'ratio.dat',/append
    printf,33,format=fmt,phase_angle_E,illum_ratio,illum_ratio_err,k,doy,am, Isun,Iearth
    close,33
endif
...
Earth_Albedo_factor=1.00            ; Scaling factor for Earth albedo
if_show=0                                       ; see a picture of the Moon
...
earth_albedo = 0     ; 0=uniform (0.3000) ,  1=albedo climatology from file, 2=time-dependent globally uniform albedo variation
...
hh  = 9           ; Observing hour UT
mnt = 29            ; Observing minute past that hour
sec = 55.0          ; Observing second past that minute
...
jdstart = JD
jdstep =  1.0/24./60.
for JD=jdstart,jdstart+1./24.,jdstep do begin

    print,format='(a,1x,f20.4)', 'JD = ', JD
    eshine_core, JD, moon_albedo, moon_BRDF, earth_albedo, earth_BRDF, filter, filterfact,CCD_out_LONG,CCD_out_DOUBLE
    writefits,strcompress('Moon_simulated_'+string(icount)+'_LONG.FIT',/remove_all),CCD_out_LONG
    writefits,strcompress('Moon_simulated_'+string(icount)+'_DOUBLE.FIT',/remove_all),CCD_out_DOUBLE
endfor
