FUNCTION   bootstrapper,pixels_in
pixels=pixels_in
n=n_elements(pixels)
...
;JD=double(liste(ipoint))
 ...
 common Y, Yobs
 common fanstuff,rad_raw,line_raw,rad_folded,line_folded,im1_folded,im2_folded,old_alfa1,old_acoeff,old_bpwr
 common erry,erry
...
; fold, but only if alfa1, aceoff or bpwr have changed 
 if (old_alfa1 eq alfa1 and old_acoeff eq acoeff and old_bpwr eq bpwr) then begin
 folded=im1_folded*(1.0d0-albedo)+im2_folded*albedo
 print,'Used the old im1 im2!'
 endif else begin
 writefits,'im1.fits',im1,mixedimageheader
 str='./justconvolve_spPFS im1.fits trialout117.fits '+string(alfa1)+' '+string(acoeff)+' '+string(bpwr)
 spawn,str;,/NOSHELL
 im1_folded=readfits('trialout117.fits',/silent)

 writefits,'im2.fits',im2,mixedimageheader
 str='./justconvolve_spPFS im2.fits trialout117.fits '+string(alfa1)+' '+string(acoeff)+' '+string(bpwr)
 spawn,str;,/NOSHELL
 im2_folded=readfits('trialout117.fits',/silent)

 folded=im1_folded*(1.0d0-albedo)+im2_folded*albedo
 old_alfa1=alfa1
 old_acoeff=acoeff
 old_bpwr=bpwr
; perhasp the next lines can be removed?????
 writefits,'mixed117.fits',mixedimage,mixedimageheader
 str='./justconvolve_spPFS mixed117.fits trialout117.fits '+string(alfa1)+' '+string(acoeff)+' '+string(bpwr)
 spawn,str;,/NOSHELL
 folded=readfits('trialout117.fits',/silent)
 endelse
 print,'in foldnfan: ',alfa1,acoeff,bpwr
...
; build a new header based on the old one along with
 ...
 xyouts,q,ystart-2.*ystep,'!7b!3      = '+string(bpwr,format='(f7.4)')
 ...
 PRO gofitFAN,albedo,erralbedo,alfa1,bpwr,pedestal,xshift,yshift,acoeff,lamda0
...
;    endif
 zodi=0.0
 SLcounts=0.0
 p = [alfa1,bpwr,pedestal,albedo,xshift,acoeff,lamda0,yshift,zodi,SLcounts]
 ...
 parinfo[1].fixed = 1	; bpwr
 parinfo[2].fixed = 0	; pedestal
 parinfo[3].fixed = 0	; albedo
 parinfo[4].fixed = 0	; xshift
 parinfo[5].fixed = 1	; acoeff
 parinfo[6].fixed = 1	; lamda0	alwasy fixed - is set by code later
 parinfo[7].fixed = 0	; yshift
 parinfo[8].fixed = 1	; ZL always fixed - is set by code later
 parinfo[9].fixed = 1	; SL always fixed - is set by code later
 ; alfa1 - the 'wing alfa'
 ...
 parinfo[2].limits(0)  = -20.0
 parinfo[2].limited(1) = 1
 parinfo[2].limits(1)  =90.0 
 ; albedo
 ...
 parinfo[4].limits(0)  = -2
 parinfo[4].limited(1) = 1
 parinfo[4].limits(1)  = 2
 ; core factor
 ...
 parinfo[7].limits(0)  = -2.
 parinfo[7].limited(1) = 1
 parinfo[7].limits(1)  = 2.
 ; zodiacal counts
...
;p(6)=1.0	; B lamda0
 ...
 parinfo[*].value = p
 ;
 w1=70	; inside edge on DS sky
 w2=60	; outside disc edge on DS disc
 ...
 actioninidcator=1
 use_cusp_angle_fan_DS_BS,actioninidcator,raw,x0(0),y0(0),radius,rad_raw,line_raw,erry,kmethod,w1,w2,w3,w4
 y=line_raw
...
print,'     alfa   bpwr  pedestal  albedo   xshift   corefac lamda0  yshifta     ZL       SL'
 ...
 ; Generate two unconvolved FITS images of the Moon for the given JD with Earth albedo 0 and 1
 get_lun,hjkl
...
;writefits,'im2_justaftercreation.fits',im2
 ...
 ;..............................................................................
 ; Version 30_faster of code version 30
 ; To Do: Must set up conditional convolution of im1 and im2 only if alfa or coef or bpwr has changed
 ; Must set up storing convolved im1 and convolved im2
 ;..............................................................................
 common stuff117,name,use1,use2,im1,im2,raw,RMSE,x0,y0,radius,nbands,w1,w2,w3,w4
 common namingstuff,JD,labelstr,markedupimage,if_smoo,if_jiggle,jiggle_ampl,paramname
 common fanstuff,rad_raw,line_raw,rad_folded,line_folded,im1_folded,im2_folded,old_alfa1,old_acoeff,old_bpwr
 ...
 common ifremoveZLandSL,ifremoveZLandSL,exptime
 common eclipticcorrdsforMoon,jdCOM,raCOM,decCOM,moonECLlonCOM,moonECLlatCOM
 common filterstuff,filter,if_want_replacedBS
 common cuspanglestuff,iflagcuspangle,cangle
 common vizualisation,ifviz
 common fittingpars,parinfo
 ...
 old_acoeff=-911
 old_bpwr=-911
; name of the PSF parameters
 paramname=['alfa1 PSF parameter','bpwr PSF parameter','pedestal PSF parameter','albedo PSF parameter','xshift PSF parameter','acoeff PSF parameter','lamda0 PSF parameter','yshift PSF parameter','ZL PSF parameter','SL PSF parameter']
 spawn,"rm -f mixed117.fits im2.fits im1.fits im2_justaftercreation.fits ItellYOUwantTHISimage.fits im1_justaftercreation.fits trialout117.fits";,/NOSHELL
 close,/all
 iflagSMK=1	; seems to be a dead flag?
 zflag=1
 ifremoveZLandSL=1
 if_want_replacedBS=0	; want model BS replaced by scaled observed BS?
 if_want_LRO=1	; 0 is Clementine, 1 is LRO
...
;.....................
 indatapath=''
 iflag_theta=1	; mystery flag
 openr,3,'special_image_to_fit_thorougly.txt'
...
close,91
         ...
         if (x0-radius gt 31 and rawncols-1-(x0+radius) gt 31 ) then begin
             ...
             im2=im2_good
             ;..........................................
             gofitFAN,albedo,erralbedo,alfa1,bpwr,pedestal,xshift,yshift,acoeff,lamda0
             print,'RMSE: ',RMSE
             print,'Albedo:',albedo,' +/- ',erralbedo
             totfl=total(raw-pedestal,/double)
             fmtstr44='(f15.7,9(1x,f9.5),1x,f10.3,1x,f15.3,2(1x,f10.7),1x,g15.6,2(1x,a),1x,a,20(1x,i1))'
             openw,63,'LRO.bootstrapperfitting_PSFisalfa1only_fastercode.txt',/append
      ;      openw,63,'LRO.bootstrapperfitting_PSF_allpars_fastercode.txt',/append
             flux=totfl/exptime
             printf,63,format=fmtstr44,JD,albedo,erralbedo,alfa1,bpwr,pedestal,xshift,yshift,acoeff,lamda0,RMSE,totfl,zodi,SLcounts,flux,name,labelstr,filter,parinfo.fixed
             close,63
 ...
 if (ifremoveZLandSL eq 1) then begin
 get_lun,grk
 openw,grk,'Moons_coordinates.dat',/append
   printf,grk,format=fmt16,jdCOM,raCOM,decCOM,moonECLlonCOM,moonECLlatCOM
 close,grk
 free_lun,grk
 endif
         endif
;    spawn,'./safekeep.scr '+string(jd,format='(f15.7)');,/NOSHELL
