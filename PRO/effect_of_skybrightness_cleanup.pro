...
;file='CLEM_and_LRO_fits_October_13_2015.numerical'
 file='CLEM_and_LRO_fits_October_13_2015.numerical_brightskiesremoved'
 data=get_data(file)
 idx=sort(data(0,*))
 data=data(*,idx)
 jd=data(0,*)
 ...
 for ifilter=0,4,1 do begin
     ...
     for ijd=0,nuniq-1,1 do begin
         ...
         for itype=0,5,1 do begin
             idx=where(jd eq uniqjd(ijd) and filter eq ifilter)
             if (idx(0) ne -1) then print,format='(f15.7,1x,a,1x,f9.4,1x,i)',uniqjd(ijd),filternames(ifilter),stddev(albedo(idx)),1
             endfor
             endfor
             endfor
 end
