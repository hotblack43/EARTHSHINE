...
; generate Poisson noise - should it be 'added'? Isn't it a 'draw from' situation?.
	folded=folded + 0.5 * PoiDev(folded)
	writefits,strcompress('OUTPUT/MLIMAGES/testim_'+string(albedo)+'.fits',/remove_all),folded
	; Dimensionaly reduce that image
	n=8
	blocked = REBIN(folded, n,n)
	openw, 73, 'data_8x8_for_ML_convolved_different_alfa_PoiDev_JD.dat', /append
	printf,73,[rebin(blocked,n*n),albedo],FORMAT='(65(F12.3,1X))'
	close,73
	endif
endfor
end
