...
; Remove linear and/or quadratic trends from data
order=2
res=linfit(indgen(n),y,yfit=yhat) & y=y-yhat
res=linfit(indgen(n),x1,yfit=yhat) & x1=x1-yhat
if (order eq 1) then begin
	res=linfit(transpose(indgen(n)),x2,yfit=yhat)
	 x2=x2-yhat
endif
if (order eq 2) then begin
	res=regress([transpose(indgen(n)),transpose(indgen(n)^2)],x2,yfit=yhat,const=const)
	x2=x2-yhat
endif
;---------------------------------------------
; Plot detrended data
plot,x1,ytitle='Detrended Sunspot #',charsize=1.4,psym=-7
plot,x2,ytitle='Detrended CO2',charsize=1.4,psym=-7
;................
...
r=correlate(yhat,y)
plot,y,ytitle='Wind stress and CS model',charsize=1.4,psym=-7,title='R='+string(R)
oplot,yhat,thick=2
