...
PRO eshine_core_20_nolroclem, JD, phase_angle, lat_lib, lon_lib, PA_lib,                 $
...
Print,"This is eshine_core_20.pro with a provision for scaling the Clementine map to Wildey levels"
...
if (moon_BRDF EQ 0) then begin
  ...
  S = (2.0/(3*!DPI)) * ( (sin(30./DRADEG) + (!DPI-30./DRADEG)*cos(30./DRADEG))/!DPI + tvalue*(1.0 - 0.5*cos(30./DRADEG))^2 )
  g30 = B*S  ; 0.228068
...
writefits,strcompress('OUTPUT/SMOOTH/lonlatSELimage_JD'+string(JD,format='(f15.7)')+'.fits',/remove_all),lonlatSELimage,header55
get_lun,chj
...
writefits,strcompress('OUTPUT/SMOOTH/Angles_JD'+string(JD,format='(f15.7)')+'.fits',/remove_all),theta_i_and_r_and_phi,header55
