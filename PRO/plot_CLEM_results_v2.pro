...
return
 ...
 godostats,0.31,a(idx),1.72,alfa(idx),'Albedo','Alfa',bias_alfa
 godostats,0.31,a(idx),0,ped(idx),'Albedo','pedestal',bias_ped
 godostats,0.31,a(idx),0,dx(idx),'Albedo','dx',bias_dx
 godostats,0.31,a(idx),0,dy(idx),'Albedo','dy',bias_dy
 print,'---------------------------------------------------'
 ;-------------------------
;!P.MULTI=[0,1,2]
 !P.MULTI=[0,2,4]
 gomakehisto,ped(idx),'Pedestal',0.000031,xra
 gomakehisto,dy(idx),'dy',0.15,xra
 gomakehisto,dx(idx),'dx',0.15,xra
 gomakehisto,rmse(idx),'RMSE',0.007,xra
 gomakehisto,alfa(idx),'Alfa',0.0018,xra
 gomakehisto,a(idx),'Albedo',0.0009,xra
 gomakehisto,da(idx),'d Albedo',0.00006,xra
;----------------------------------
 getstats,a(idx),medval,pctilelow,pctilehi,robSD
 fmtstr12='(f15.7,1x,a5,1x,i4,1x,f9.4,1x,g9.4,2(1x,f9.4))'
 fmtstr13='(f15.7,a,a5,a,i4,a,f9.4,a,g9.4,a)'
 openw,iuygy,'MASTER_outputlist_from_bagging.txt',/append
        print,format=fmtstr12,uniqueJDs(i),filternames(0),N,medval,robSD,pctilelow,pctilehi
 printf,iuygy,format=fmtstr12,uniqueJDs(i),filternames(0),N,medval,robSD,pctilelow,pctilehi
 printf,66,format=fmtstr13,uniqueJDs(i),' & ',filternames(0),' & ',N,' & ',medval,' & ',robSD,' \\'
 close,iuygy
 free_lun,iuygy
 endif else begin
 print,'Too few results for ',uniqueJDs(i),' - skipping for now.'
 endelse
