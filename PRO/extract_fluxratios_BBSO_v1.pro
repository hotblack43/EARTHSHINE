...
PRO get_BBSOalbedos,JD,DCRalbedo,LINalbedo,LOGalbedo,iflag
 iflag=0
...
;stack=[[[DCR]],[[ideal]],[[BBSOlin]],[[BBSOlog]],[[lon]],[[lat]]]
 lonmin=70.0
 lonmax=80.0
 latlim=40.0
...
if (DS_BBSOlog gt BS_BBSOlog) then swapem,DS_BBSOlog,BS_BBSOlog,delta_DS_BBSOlog,delta_BS_BBSOlog
 ...
 iflag=1
 tvscl,[ideal/max(ideal),dcr/max(dcr)]
 return
 end
 ...
 mphase,JD,k
 get_BBSOalbedos,JD,DCRalbedo,LINalbedo,LOGalbedo,iflag
 if (iflag ne 0) then printf,56,format='(f15.7,3(1x,f9.5),1x,f6.1,1x,f6.3)',JD,DCRalbedo,LINalbedo,LOGalbedo,phase,k
 if (iflag ne 0) then print,format='(f15.7,3(1x,f9.5),1x,f6.1,1x,f6.3)',JD,DCRalbedo,LINalbedo,LOGalbedo,phase,k
