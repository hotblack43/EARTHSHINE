...
;
fmt_str='(11(1x,g7.2),2(1x,i4),1x,a)'
header=' Tot/pix   Poi_D   Poi_B   DF_D    DF_B    FF_D    FF_B    SL_D    SL_B    DE   Tot_np_ni  np   ni      type'
...
delta_SL/I_bright,delta_DE_fractional,sqrt(f_tot_squared_mean),n_pix,n_ims,'     rnd err'

;printf,1,format=fmt_str,sqrt(f_tot_squared),delta_ID/I_low,delta_IB/I_bright,delta_DF/I_low,$
printf,1,sqrt(f_tot_squared),delta_ID/I_low,delta_IB/I_bright,delta_DF/I_low,$
delta_DF/I_bright,delta_FF/I_low,delta_FF/I_bright,delta_SL/I_low,$
...
printf,1,bias_tot,delta_ID/I_low,delta_IB/I_bright,delta_DF/I_low,$
delta_DF/I_bright,delta_FF/I_low,delta_FF/I_bright,delta_SL/I_low,$
...
PRO plotter,randomname,ytit_str,tit_str,ytit_str_abs,tit_str_abs
...
xticknames=['Poi-D','Poi-B','DF-D','DF-B','FF-D','FF-B','SL-B','SL-D','DE']
;
plot,array,y,psym=7,ytitle=ytit_str,title=strcompress(tit_str+' Npi:'+string(fix(n_pix))+' Nim:'+string(fix(n_ims))), $
xtickv=array,xtickname=xticknames,xticks=8,xminor=1,xrange=[-1,9], $
/ylog,charsize=1.2,yrange=[min(y(where(y ne 0))),100]
; abs errors
plot,array,x(1:9),psym=7,ytitle=ytit_str_abs,title=strcompress(tit_str_abs+' Npi:'+string(fix(n_pix))+' Nim:'+string(fix(n_ims))), $
xtickv=array,xtickname=xticknames,xticks=8,xminor=1,xrange=[-1,9], $
/ylog,charsize=1.2,yrange=[min(y(where(y ne 0))),max(y)]
return
end
...
!P.MULTI=[0,2,2]
;------------------------
for pixpower=0.,4.0,0.35 do begin
...
randomname='one_shooter_random_error_budget.dat'
ytit_str='Relative contribution of term (%)'
tit_str='Random error budget One-shooter'
ytit_str_abs='Term (pixels)'
tit_str_abs='Abs. rand. error budget One-shooter'
plotter,randomname,ytit_str,tit_str,ytit_str_abs,tit_str_abs

randomname='two_shooter_random_error_budget.dat'
ytit_str='Rel. contribution of term (%)'
tit_str='Ran. error budget Two-shooter'
ytit_str_abs='Term (pixels)'
tit_str_abs='Abs. rand. error budget Two-shooter'
plotter,randomname,ytit_str,tit_str,ytit_str_abs,tit_str_abs

endfor	; end of pixpower loop
end
