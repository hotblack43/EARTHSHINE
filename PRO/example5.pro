; fits Model 1 to data and shows 2-D histograms, make slist of regressions coefficients
;=================================
...
idx=where(SALgridded gt 0.15 and lsmask eq 1 and latgrid ge 30) 
...
printf,w,RMSE,911
tstr=strmid(CFCfile,45,6)+' SAL>0 & over land. lat > 30'
;plot,yhat,albedo(idx),ytitle='Model albedo',xtitle='TRS/TIS',title=strcompress(tstr)	$
;		,psym=3,xrange=[0,1],yrange=[0,1],xstyle=1,ystyle=1
scatter_col,yhat,albedo(idx),minx=0.,miny=0.,maxx=1.0,maxy=1.0,binx=0.01,biny=0.01,color_bar=color_bar,ytitle='Model albedo',xtitle='TRS/TIS',title=strcompress(tstr)
oplot,[0,1],[0,1]
endif
endfor
close,1
