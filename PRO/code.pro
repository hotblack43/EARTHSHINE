...
;
 ...
 xtit='Julian Day'
;..
 ;jd=jd-julday(1,1,1997)
 ;xtit='Days since 1/1/1997'
;..
 ...
 R=diameter/2.
 arg=dist/R
 alfa=90.+acos(arg)/!dtor	; in degrees
 e=R*cos((90.-alfa)*!dtor)	; distance to terminator
 beta=90.-alfa			; in degrees
 ; set filter angle
 filter_angle=PA+90
 times=fix(filter_angle/180.)
 filter_angle=filter_angle - times*180.
 ;
 ...
 offset(idx)=-R(idx)*cos((90+alfa(idx)+beta(idx)/2.)*!dtor)	; offset in degrees	East is Positive
 ;--------------------------------------------------------------------------
; correct for flips of the filter
offset=offset*(-1)^times
;
 offset=offset/3600.0	; offset in degrees now
  kdx=where(offset gt 0)
 offset(kdx)=-offset(kdx)	; FUDGE, but it works
;
 openw,12,'p.dat'
...
; select just the situations that are good to observe
 ...
 plot,psym=-7,jd(kdx),PA(kdx),xtitle=xtit,ytitle='SS Position Angle (CCW)',xstyle=1
oplot,[!X.CRANGE],[180,180]
 plot,psym=-7,jd(kdx),dist(kdx),xtitle=xtit,ytitle='SS distance',xstyle=1
oplot,[!X.CRANGE],[0,0]
 plot,psym=-7,JD(kdx),offset(kdx),xtitle=xtit,Ytitle='Filter Offset from disc centre',xstyle=1
oplot,[!X.CRANGE],[0,0]
 plot,psym=-7,JD(kdx),filter_angle(kdx),xtitle=xtit,Ytitle='Filter Angle wrt North',xstyle=1
oplot,[!X.CRANGE],[0,0]
; select just the situations that are good to observe
