pro extract_p_T10m_mlo,jd_want,p_out,t_out
 ...
 if (mlo_flag ne 314) then begin
     ...
     print,'reading ...'
;    restore,'/data/pth/DATA/moon/idlsave.dat'
 ...
 t_out=interpol(temp_at_10m,jdmetcorr,jd_want)
 return
 end
 ...
 if (if_meteo_correct eq 1) then begin
     extract_p_T10m_mlo,jd,p_out,t_out
	p_out=p_out/1013.25*760.0	; now in mm Hg
	print,format='(a,1x,f15.7,2(1x,f9.3))','JD p T10m: ',jd,p_out,t_out
     am = airmass(JD, RAmoon*!dtor, $
     DECmoon*!dtor, lat*!dtor, lon*!dtor,wave,p_out,t_out,relhum)
     endif
