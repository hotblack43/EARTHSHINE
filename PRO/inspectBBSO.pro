...
return
 ...
 viz=0
 ;---------------------------------------------------------------------------------------
 for iflag=1,2,1 do begin	; set 1 or 2 for 3/5ths or 2/3rds of radius to place DS patch
 ww=9
 nn=512
 im=fltarr(nn,nn)
 mask=fltarr(nn,nn)
 ...
 idealpath='/data/pth/RUN23/SYNETHICIMAGES/'
 ; point to where the synethic observed images are 
 obspath='NOISEADDED_161/OUTPUT/'
 ; point to where all the output is
 cleanedpath=obspath+'/BBSO_CLEANED/'
 cleanedpath=obspath+'/BBSO_CLEANED_LOG/'
 ; now collect all images
 files=file_search(strcompress(obspath+"/Lu*.fit",/remove_all),count=n)
 openw,77,strcompress('threeetc_'+string(iflag)+'.dat',/remove_all)
...
; find the DS intensity in the ideal image
	gofindDSBS,ideal_im,ideal_im,x0,y0,radius,cg_x,cg_y,ww,dummy,DSideal,iflag
; get DS,BS ratio in the observed image 
	gofindDSBS,observed,observed,x0,y0,radius,cg_x,cg_y,ww,dummy,DSobs,iflag
; get DS,BS ratio in the cleanedup image 
	gofindDSBS,im,im,x0,y0,radius,cg_x,cg_y,ww,dummy,DScleaned,iflag
	print,format='(5(1x,f9.3),1x,i3,3(1x,f9.2),1x,a)',x0,y0,radius,cg_x,cg_y,ww,DScleaned,DSideal,DSobs,fitsname
        printf,77,format='(6(1x,f12.6),a,1x,a)',DScleaned,DSideal,DSobs,x0,y0,radius,' ',fitsname
     endfor
 close,77
 endfor
 end
