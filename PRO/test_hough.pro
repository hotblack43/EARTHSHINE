...
im=readfits('Image_1.fit')
edge=sobel(im)
;
for ishift=1,10,1 do begin
im_to_use=shift(median(edge,7),20*randomu(seed),20*randomu(seed))
imSiz=SIZE(im_to_use)
		kpts=WHERE(im_to_use gt median(im_to_use))
            xpts=kpts MOD imSiz[1]
            ypts=kpts/imSiz[1]
R=[104.4]
WINDOW,/FREE,XSIZE=imSiz[1],YSIZE=imSiz[2],TITLE='Circles'
loadct,31
decomposed=0
contour,im_to_use,/cell_fill
circles=CircleHoughLink(xpts,ypts,RADIUS=R,VERBOSE=1)
