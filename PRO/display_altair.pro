...
files=file_search('DS_*ALTAIR*.fits',count=n)
ic=0
for i=0,n-1,1 do begin
im=double(readfits(files(i),h,/silent))
idx=where(finite(im) ne 1)
jdx=where(finite(im) eq 1)
if (idx(0) ne -1) then im(idx)=max(im(jdx))
...
writefits,'renameMEorLOOSEme.SIRIUS_stacked_V.fits',ref/max(ref)*50000.0d0
end
