...
return
 ...
 dectime=999.99
 if (yy ne 0) then dectime=julday(mm,dd,yy,hh,mi,se)
 return
 end


files=file_search('/media/SAMSUNG/MOONDROPBOX/DARKS/*DARK*.fits',count=n)
openw,32,'darkies.dat'
for i=0,n-1,1 do begin
im=readfits(files(i),header,/silent)
get_info_from_header,header,'DMI_ACT_EXP',exptime
get_info_from_header,header,'FRAME',jd
get_info_from_header,header,'UNSTTEMP',T
print,format='(f16.7,1x,i6,1x,f9.3,1x,f9.3)',jd,median(im),exptime,T
printf,32,format='(f16.7,1x,i6,1x,f9.3,1x,f9.3)',jd,median(im),exptime,T
endfor
close,32
end
