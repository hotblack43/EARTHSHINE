PRO condition,jd,ext,err,lim1,lim2,lim3,lim4,volcanic
; remove smooth
ext=ext-smooth(ext,13,/edge_truncate)
if (volcanic eq 0) then idx=where(not ((jd ge lim1 and jd le lim2) or (jd ge lim3 and jd le lim4)))
if (volcanic eq 1) then idx=where((jd ge lim1 and jd le lim2) or (jd ge lim3 and jd le lim4))
...
keyfile=['SW_MJJAS_NDJFM.dat','SBC_list_NDJFM_MJJASd.dat','FD_list_all_WI_SU_months.dat','SPE_keydates_yy_doy.dat']
...
for ifil=1,nfiles-1,1 do begin
...
if (volcanic eq 2) then volc_str='All years '
 condition,jd,ext,err,julday(1,6,1991),julday(1,6,1994),julday(1,1,1982),julday(6,1,1985),volcanic
...
xx=fltarr(2*half_win+1)
yy=fltarr(2*half_win+1)
ww=fltarr(2*half_win+1)
zz=fltarr(2*half_win+1)
j=0
for i=-half_win,half_win,1 do begin
	xx(j)=i
	ww(j)=0.0
	yy(j)=0.0
	idx=where(x ge i and x le i+1)
	if (idx(0) ne -1) then yy(j)=mean(y(idx))
	if (idx(0) ne -1 and n_elements(idx) gt 2) then ww(j)=stddev(y(idx))/sqrt(n_elements(idx)-1)
	zz(j)=n_elements(idx)
	j=j+1
endfor
