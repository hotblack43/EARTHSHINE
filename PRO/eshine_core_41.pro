...
;
; Version 41 - is like version 40 except user can select Clementine or LRO
;              lunar albedo map - in the case of LRO also select the wavelength band
;              These selections are done through files that were set up during 
;              running of the calling code
;
...
PRO eshine_core_41, JD, phase_angle, lat_lib, lon_lib, PA_lib,                 $
...
Print,"This is eshine_core_41.pro "
...
close,hgjfrte
free_lun,hgjfrte
; read the switch for Clementine or LRO lunar albedo map
if_want_LRO=0
get_lun,htrdg3
openr,htrdg3,'DOyouWantLROorClementine.txt'
readf,htrdg3,if_want_LRO
close,htrdg3
free_lun,htrdg3
if (if_want_LRO eq 0) then print,'You have selected to use the Clementine albedo map'
if (if_want_LRO eq 1) then begin
	print,'You have selected to use the LRO albedo map'
	print,'You have stated that your filter is: ',filter_str
endif
...
get_lun,uyfyrtd6345fvgy
openw,uyfyrtd6345fvgy,'libration_info.dat'
 print,JD, RAs, DECs, DISTs, RAm, DECm, DISTm, lat_lib, lon_lib, PA_lib
 printf,uyfyrtd6345fvgy,format='(f15.7,3(1x,f9.4))',JD, lat_lib, lon_lib, PA_lib
close,uyfyrtd6345fvgy
endif else begin
  lat_lib = 0.0
  lon_lib = 0.0
  PA_lib  = 0.0
...
if (if_want_LRO ne 1) then  begin
...
;......................................................
; 1 -> LRO wavelength-dependent map, filled in with scaled Clem map above lat 70
if (if_want_LRO eq 1) then begin
        if (filter_str eq 'B' or filter_str eq '_B_') then PNORMMOON=reform(readfits('Eshine/LRO_interpolated_B_464.fits'))
        if (filter_str eq 'V' or filter_str eq '_V_') then PNORMMOON=reform(readfits('Eshine/LRO_interpolated_V.fits'))
        if (filter_str eq 'VE1' or filter_str eq '_VE1_') then PNORMMOON=reform(readfits('Eshine/LRO_interpolated_VE1_569.fits'))
        if (filter_str eq 'VE2' or filter_str eq '_VE2_') then PNORMMOON=reform(readfits('Eshine/LRO_interpolated_VE2_742.fits'))
        if (filter_str eq 'IRCUT' or filter_str eq '_IRCUT_') then PNORMMOON=reform(readfits('Eshine/LRO_interpolated_IRCUT_560.fits'))
endif
;......................................................
; PNORMMOON = float(X.field0001)
endif
...
for ira=0,359 do begin
  ...
  if (BRDFearth[ilon,ilat] EQ 0) then begin
    ...
    if (AoI LE !DPI/2) AND (AoR LE !DPI/2) then begin
      BRDFse = OMEGAearth[ilon,ilat]/!DPI*(cos(AoI)*cos(AoR))^(minnaert_k-1.)
    endif else begin
      BRDFse = 0.0
    endelse
