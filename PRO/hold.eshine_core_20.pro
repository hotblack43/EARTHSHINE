 PRO get_mixed_Lambert_Hapke_BDRF,AoIs,AoR,OMEGA,fphHapke63,BRDFsm,mixingfactor
 ;  will 'mix' Lambert and Hapke BDRF for the moon using a weighting factor 
 ; note that 0 <= mixingfactor <= 1 is assumed
      ; Lambert
      if (AoIs LE !DPI/2) AND (AoR LE !DPI/2) then begin
        BRDFsmLambert = OMEGA/!DPI
      endif else begin
        BRDFsmLAmbert = 0.0
      endelse
      ; Hapke -63
      if (AoIs LT !DPI/2) AND (AoR LT !DPI/2) then begin
        BRDFsmHapke =  OMEGA * fphHapke63 * 1.0/(cos(AoIs)+cos(AoR))
      endif else begin
        BRDFsmHapke = 0.0
      endelse

     BRDFsm=(1.-mixingfactor)*BRDFsmLambert+mixingfactor*BRDFsmHapke
...
; for the 'g' factor in Hapkes formula. A mixing between Hapke and LAmbert is allowed.
; and also returns a 'mask' showing su the sunlit part of the Moon
...
PRO eshine_core_20, JD, phase_angle, lat_lib, lon_lib, PA_lib,                 $
...
Print,"This is eshine_core_20.pro"
...
lonlatSELimage=fltarr(imsize,imsize,2)
...
; generate map of selenographic coordinates
  ...
  if hitMoon then begin
    ...
    ; this is a test of mixing Hapke and Lambert by a weighting factor:
    get_mixed_Lambert_Hapke_BDRF,AoIs,AoR,OMEGA,fphHapke63,BRDFsm,mixingfactor
...
writefits,strcompress('lonlatSELimage_JD'+string(JD,format='(f15.7)')+'.fits',/remove_all),lonlatSELimage
