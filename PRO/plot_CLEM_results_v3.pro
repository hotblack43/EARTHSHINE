PRO getstats,a_in,medval,pctilelow,pctilehi,robSD
 ...
 xyouts,/data,x0,y0,'Median: '+string(median(a),format='(f8.5)'),charsize=1.6
 xyouts,/data,x0,y0-offset,'SD: '+string(robust_sigma(a),format='(f8.4)'),charsize=1.6
 return
 end
...
;PRO godostats,y,x,ystr,xstr,res_slope
 ...
 psym=1,charsize=1.4,thick=3,title=jdstr
 ...
 dx=reform(data(6,*))
 dy=reform(data(7,*))
 idx=where(rmse le 0.3 and abs(dx) lt 2 and abs(dy) lt 2)	; filter out the junk
 data=data(*,idx)
...
;        godostats,a(idx),dy(idx),'Albedo','dy',dysigmas
         ...
         gomakehisto,dy(idx),'dy',binw*2*60.,xra
         gomakehisto,dx(idx),'dx',binw*2*60.,xra
         gomakehisto,rmse(idx),'RMSE',binw*6,xra
         gomakehisto,alfa(idx),'Alfa',binw*2,xra
         gomakehisto,a(idx),'Albedo',binw,xra
         gomakehisto,da(idx),'d Albedo',binw/8.,xra
         ;----------------------------------
