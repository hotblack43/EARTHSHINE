PRO plot3things,observed,trialim,cutoffval 
 ...
 ;------------------------------------------------------------------------
 ; version 5 of Empirical Forward Method - will loop over input files
 ; This version just like version 3, but is intended for tests on synthetic images
 ;------------------------------------------------------------------------
 !P.CHARSIZE=2
 common ims,observed,source,residual,mask,trialim,cleanup
 common errs,errorwholeimage,errorinabox,idealerrorinabox,b,idealbiasinbox
 ...
 get_lun,ww
 get_lun,uu
 openw,ww,'collected_output_EFM_v5.txt'
 openw,uu,'collected_output_EFM_v5_ideal.txt'
 ;for cutoffval=75,100,10 do begin
 cutoffval=75
 ...
 ;for pedestal=100,10,-10 do begin
	pedestal=400.0	; det faktiske pedestal brugt i det konstrurede billede - ikke et startgæt
     ; generate a synthetic observed image
     spawn,'cp quitespecialMOONideal.fits ideal_in.fits'
     ...
     ;spawn,'./syntheticmoon usethisidealimage.fits synth_observed_1p6.fits 1.6 1 '+string(seed)
     ;spawn,'./syntheticmoon usethisidealimage.fits synth_observed_1p7.fits 1.7 1 '+string(seed)
     spawn,'./syntheticmoon usethisidealimage.fits synth_observed_1p8.fits 1.8 1 '+string(seed)
     ...
     ; generate the '1/75th' source image
     im=observed
     idx=where(im lt max(smooth(im,3))/cutoffval)
     im(idx)=0
     writefits,'source.fits',im
     ...
     y0=255.0
     radius=140.0d0
     radius=radius+5	; mask slightly larger than lunar disc
     ...
     fixed:0, limited:[0,0], limits:[0.D,0],step:1.0d-5}, 2)
     parinfo[0].fixed = 0
     parinfo[1].fixed = 0
     ; a
     ...
     ; Note, above, the subtraction of a from observed - this is so that the added 
     ; pedestal, and any pedestal-like sky residual, will be removed when measuring the BS
; also do this for the ideal image
     iflag=1
     gofindDSandBSinboxes,idealused,idealused,x0,y0,radius,cg_x,cg_y,w,BSideal,DS45ideal,iflag
     iflag=2
     gofindDSandBSinboxes,idealused,idealused,x0,y0,radius,cg_x,cg_y,w,BSideal,DS23ideal,iflag
     ;.................................................................................
     ; print out some results
     fmt='(2(1x,f14.10),4(1x,f15.5),3(1x,f8.3),2(1x,i5))'
     printf,ww,format=fmt,alfa,a,BS,total(observed-a,/double),DS23,DS45,x0,y0,radius,pedestal,cutoffval
     printf,uu,format=fmt,alfa,a,BSideal,total(idealused,/double),DS23ideal,DS45ideal,x0,y0,radius,pedestal,cutoffval
 ;    endfor	; end of pedestal loop
 ;   endfor	; end of cutoff loop
 close,ww
 free_lun,ww
 close,uu
 free_lun,uu
 writefits,strcompress('S_'+string(iloop)+'.fits',/remove_all),trialim
