PRO goaddpoisson,im,n
 writefits,'input_goaddpoisson.fits',im
...
if (im(i,j) lt 20) then begin
	im(i,j)=max([im(i,j)+randomn(seed,/double)/400.,0])
endif else begin
arg=n*im(i,j);+10*n
rnd=max([randomu(seed,poisson=arg,/double)/float(n),0])
im(i,j)=rnd
endelse
endfor
endfor
...
PRO	get_fake_exptime,x0_y0_radius,im,x0,y0,radius,exptime
idx=where(im(*,256,1) ne 0)
x0=511./2.
y0=511./2.
exptime=1.0d
radius=(max(idx)-min(idx))/2.
return
end

PRO forceimageflipsoDSisatleft,im
; will only work on images with x0,y0 at 255,255
; do not use for real observed images
left=total(im(0:255,*,0))
right=total(im(256:511,*,0))
if (left lt right) then return
...
stop
 ...
 common namingstuff,JD,labelstr,markedupimage,if_jiggle,jiggle_ampl,paramname
...
;str='./justconvolve_spPFS mixed117.fits trialout117.fits '+string(alfa1)+' '+string(acoeff)+' '+string(bwpr)
 str='./justconvolve_spPFS_special mixed117.fits trialout117.fits '+string(alfa1)+' '+string(acoeff)+' '+string(bwpr)
 spawn,str;,/NOSHELL
...
writefits,'im2replaced.fits',im2
 ...
 zodi=0.0
 SLcounts=0.0
 p = [alfa1,999,pedestal,albedo,xshift,999,lamda0,yshift,zodi,SLcounts]
...
;p(6)=1.0	; B lamda0
 ...
 parinfo[*].value = p
 ;
 w1=90	; inside edge on DS sky
 w2=60;100	; outside disc edge on DS disc
...
print,' before use_cusp... 2 ',raw(*,256)
 ...
 niter=500
 erry=erry*0.0+1.0
 parms = MPFITFUN('foldnFAN', X, Y, 0.43*erry/sqrt(8.1), p, parinfo=parinfo, PERROR=sigs,niter=niter,covar=covariance,STATUS=status)
 ...
 alfa1=parms(0)
 name17=strcompress("PSF_testing_alfa_"+string(alfa1)+".fits",/remove_all)
 spawn,"mv PSF_testing.fits "+name17
 bwpr=parms(1)
 pedestal=parms(2)
 albedo=parms(3)
 xshift=parms(4)
...
print,'     alfa   bwpr  pedestal  albedo   xshift   corefac lamda0  yshifta     ZL       SL'
 ...
 close,38
 openw,39,'covariance_matrix.dat'
print,'     alfa   bwpr  pedestal  albedo   xshift   corefac lamda0  yshifta     ZL       SL'
 printf,39,format='('+string(n_elements(parms))+'(1x,g8.2))',covariance
 close,39
 erralbedo=sigs(3)
 return
 end
 ...
 free_lun,hjkl
 ;...get the image
 spawn,'idl go_get_particular_synthimage_118.pro';,/NOSHELL
 ...
 ;..............................................................................
 ; Version 2 of code that fits ideal convolved images to images generated 
 ; with the code to generate ideal images - not necessarily with all things the same.
 ; Much like version 1 of this code, but does 'bagging' on each image to 
 ; reduce degeneracy-induced scatter.
 ;..............................................................................
 common stuff117,name,use1,use2,im1,im2,raw,RMSE,x0,y0,radius,nbands,w1,w2,w3,w4
 common namingstuff,JD,labelstr,markedupimage,if_jiggle,jiggle_ampl,paramname
 common fanstuff,rad_raw,line_raw,rad_folded,line_folded
...
; name of the PSF parameters
 paramname=['alfa1 PSF parameter','bwpr PSF parameter','pedestal PSF parameter','albedo PSF parameter','xshift PSF parameter','acoeff PSF parameter','lamda0 PSF parameter','yshift PSF parameter','ZL PSF parameter','SL PSF parameter']
 ...
 if_want_LRO=0
 if_stackimages=1	; are the images in stacsk with lon/lat etc?
...
;.....................
 ...
 openr,3,'JDs_halotrials.txt'
;openr,3,'JDs_halotrials_synhimages.txt';'JDs_halotrials.txt'
;openr,3,'special_image_to_fit_thorougly.txt';'JDs_halotrials.txt'
...
;    filter=get_filter_from_filename(name)
     ...
     if (illfrac lt 0.5) then begin
        ...
        forceimageflipsoDSisatleft,im
	get_fake_exptime,x0_y0_radius,im,x0,y0,radius,exptime
         if (if_stackimages eq 1) then im=reform(im(*,*,0))	; for stacks
         markedupimage=im
         if (ifviz eq 1) then tvscl,markedupimage
...
close,91
         ...
         if (x0-radius gt 31 and rawncols-1-(x0+radius) gt 31 ) then begin
             ...
             fmtstr44='(f15.7,9(1x,f9.5),1x,f10.3,1x,f15.3,2(1x,f10.7),1x,g15.6,2(1x,a),1x,a,20(1x,i1),1x,i4)'
             openw,63,'CLEM.bagging.txt',/append
             flux=totfl/exptime
             printf,63,format=fmtstr44,JD,albedo,erralbedo,alfa1,bwpr,pedestal,xshift,yshift,acoeff,lamda0,RMSE,totfl,zodi,SLcounts,flux,name,labelstr,filter,parinfo.fixed,status
             close,63
