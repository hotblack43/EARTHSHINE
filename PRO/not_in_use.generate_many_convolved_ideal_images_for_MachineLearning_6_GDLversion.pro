...
free_lun,hjkl
;================================================
spawn, /SH,'gdl go_get_particular_synthimage_16_for_ML.pro'
get_lun,iuhgyuygf
...
PRO generate_many_convolved_ideal_images_for_MachineLearning_6_GDLversion
;---------------------------------------------------------
...
for itrial=1,nalbedo,1 do begin
	...
	; convolve it
        name14 = strcompress('/app/OUTPUT/TEMP/mixed117.fits')
        print,name14
	writefits,name14,im1,mixedimageheader
        alfa1 = randomu(seed)*0.2+1.7
 	str='./justconvolve_scwc /app/OUTPUT/TEMP/mixed117.fits /app/OUTPUT/TEMP/trialout117.fits '+string(alfa1)+' '+string(corefactor)+' '+string(rlimit)
 	spawn,/SH,str
        print,'managed to convolve!'
 	folded=readfits('/app/OUTPUT/TEMP/trialout117.fits')
...
; generate Poisson noise - should it be 'added'? Isn't it a 'draw from' situation?.
	...
	writefits,strcompress('/app/OUTPUT/MLIMAGES_100SUMMED/testim_'+string(albedo)+'_'+string(illfrac)+'_.fits',/remove_all),folded
	writefits,strcompress('/app/OUTPUT/MLIMAGES_100SUMMED/UNMODIFIED/testim_'+string(albedo)+'_'+string(illfrac)+'_.fits',/remove_all),im1
	endif
endfor
end
