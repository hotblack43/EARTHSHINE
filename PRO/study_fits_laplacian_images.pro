...
common normalize,obs_total
 file1='observed_image_JD2456045.8521221.fits'
 ifuse=1	; i.e. we are on observations now
 applyLaplacian,file1,obs_ratio,ifuse
 ; then overplot the synethtic models
 files=file_search('deal_albedo_0p*')
 nfiles=n_elements(files)
 !P.MULTI=[0,1,1]
 openw,58,'low_high_ratio.dat'
 ...
 close,58
 data=get_data('low_high_ratio.dat')
 data(0,*)=data(0,*)/2.0d0	; div by 2 
 n=n_elements(data(0,*))
 plot,0.1+findgen(n)/10.,data(0,*),psym=7,xstyle=3,charsize=1.9,ystyle=3,xtitle='Albedo',ytitle='High/low',yrange=[0,max(data(0,*))]
 ;plot,0.1+findgen(n)/10.,data(1,*),psym=7,xstyle=3,charsize=1.9,ystyle=3,xtitle='Albedo',ytitle='BS/DS',yrange=[0,max(data(1,*))]
 xx=0.1+findgen(n)/10.
 yy=data(0,*)
