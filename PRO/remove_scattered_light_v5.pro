...
angle=atan((yy-y0)/(xx-x0))/!dtor
xline=xx
yline=yy
idx=where(yline gt l(1)/2. and xline le l(0)/2.)
angle(idx)=angle(idx)+180.
idx=where(yline le l(1)/2. and xline le l(0)/2.)
angle(idx)=angle(idx)+180.
idx=where(yline le l(1)/2. and xline gt l(0)/2.)
...
common mooncoords,radius,moon_coords
...
common fitedresults,P
common type,typeflag
if (strupcase(method_str) ne 'LINEAR') then begin
; gaussian case
if (STRUPCASE(typeflag) eq "GAUSSIAN") then begin
...
MKHDR, header, difference
WRITEFITS, 'Corrected_image_linear.fit', difference,header
WRITEFITS, 'Corrected_image_linear_LONG.fit',long( difference),header
endif
return
end
