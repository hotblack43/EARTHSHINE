...
PRO plot3things,headerstring,observed,trialim,cuttoffval,alfa 
...
; object function for case:L we want the halo to be fitted to the DS sky
...
print,'p:',p
;.............................................................
; In this version of the fitted halo we force flux conservation as 
; well as passaga through a sky-point near the DS edge; this specifices a and b uniquely
;
; Estimate offset 81) and factor (b) on the P*I image (I=image source)
; Use he convetion that NO sky pedestal has been subtracted from the observed image
;
...
getthesourceimagefromtheobservedimage,observed,im,a
; im is the source and needs to be convolved with the PSF
 str='./justconvolve source.fits source_folded_out.fits '+string(alfa)
 spawn,str
 source_folded=readfits('source_folded_out.fits',/silent)
; solve for a and b in the anchored model
totobserved=mean(observed,/double)
totpatchfolded=mean(source_folded(xSkyPath-w:xSkyPath+w,ySkyPatch-w:ySkyPatch+w),/double)
totfolded=mean(source_folded,/double)
totpatchobserved=mean(subim,/double)
print,'totobserved,totfolded,totpatchfolded,totpatchobserved: ',totobserved,totfolded,totpatchfolded,totpatchobserved
a = -(totobserved*totpatchfolded-totfolded*totpatchobserved)/(totfolded-totpatchfolded)
b = (totobserved-totpatchobserved)/(totfolded-totpatchfolded)
print,'In minimize_me_2 I solve (a,b)=',a,b
;.............................................................
...
; get residuals wrt observed image
 ...
 print,'In trial cleanedup image SKy patch has mean: ',mean(cleanup(xSkyPath-w:xSkyPath+w,ySkyPatch-w:ySkyPatch+w))
...
return
 ...
 ;--------------------------------------------------------------------
 ; RUNNER version 6. EFM method.
 ; Like version 5, but has option for 'anchoring' the halo on the DS sky, 
 ; as well as flux conservation. See if_want_anchor, below.
 ;--------------------------------------------------------------------
 !P.CHARSIZE=2
 common ims,observed,source,residual,mask,trialim,cleanup
 common errs,errorwholeimage,errorinabox,idealerrorinabox,b,idealbiasinbox
...
;..................................................
 ...
 ifviz=0
 cuttoffval=90
 if_want_anchor=1	; should the minimizer work on 
;           residuals (choose 0) or 'anchored residuals' (choose 1)?
;..................................................
...
; path to input files
 str=strcompress(lowpath+'DARKCURRENTREDUCED/SELECTED_1/*245*.fits',/remove_all)
 files=file_search(str,count=nfiles)
 print,'Found m files: ',nfiles
...
print,'Starting guess a,alfa=',a,alfa
 ...
 get_mask,maskFLIPneeded,x0,y0,radius+3,mask
 if (ifviz eq 1) then tvscl,[im/max(im),mask/max(mask)]
 print,'Want to write out the mask!'
 writefits,'mask.fits',mask
 print,'I managed to write out the mask!'
...
;    weights=observed*0.0+1.0
     ...
     if(if_want_anchor eq 0) then begin
     ; print,parinfo
     results = MPFIT2DFUN('minimize_me', X, Y, Z, weights=weights, $
     PARINFO=parinfo,perror=sigs,STATUS=mpfit_status_info,xtol=1e-11)
     endif else begin
     ; set up the PARINFO array - indicate double-sided derivatives (best)
     ...
     print,'STATUS=',mpfit_status_info
if (mpfit_status_info ne -18) then begin
...
device,xsize=18,ysize=24.5,yoffset=2
 ...
 plot3things,basicfilename,observed,trialim,cuttoffval,alfa 
 device,/close
 set_plot,'x'
