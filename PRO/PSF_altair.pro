...
PRO get_exptime,header,exptime
 ...
 PRO findclosestDARKS,ALTAIRnamelist,ALTAIRtimelist,DARKnamelist,DARKtimelist,LISTofbestDARKnames
 ; looking in ALTAIRtimelist will find closest two DARKS and place their names on a list
 nTARGET=n_elements(ALTAIRtimelist)
 ic=0
 for i=1,nTARGET-1,1 do begin
     d=(DARKtimelist-ALTAIRtimelist(i))
     lastone=where(d lt 0)
     lastone=lastone(n_elements(lastone)-1)
     firstone=where(d gt 0)
     firstone=firstone(0)
     d1=DARKtimelist(lastone)-ALTAIRtimelist(i)
     d2=DARKtimelist(firstone)-ALTAIRtimelist(i)
     print,'d1,d2: ',d1*24.*60.,d2*24.*60.,' minutes.'
     maxminutesold=4
     if (abs(d1) le 1./24./60.*maxminutesold and abs(d1) le 1./24./60.*maxminutesold) then begin
     if (ic eq 0) then LISTofbestDARKnames=[DARKnamelist(lastone),ALTAIRnamelist(i),DARKnamelist(firstone)]
     if (ic gt 0) then LISTofbestDARKnames=[[LISTofbestDARKnames],[DARKnamelist(lastone),ALTAIRnamelist(i),DARKnamelist(firstone)]]
     ic=ic+1
 endif
     endfor
 return
 ...
 ; Code to carefully subtract scaled DARK frames from ALTAIR frames
 common paths,path
 spawn,'grep DARK '+'ALTAIR.list'+' > DARKS.txt'
 spawn,'grep ALTAIR '+'ALTAIR.list'+' > TARGET.txt'
 getit,'DARKS.txt',DARKnamelist,DARKtimelist,nDARKS
 getit,'TARGET.txt',ALTAIRnamelist,ALTAIRtimelist,nTARGET
 ;
 findclosestDARKS,ALTAIRnamelist,ALTAIRtimelist,DARKnamelist,DARKtimelist,LISTofbestDARKnames
 ...
 for i=0,n-1,1 do begin
     ...
     l=size(im)
     if (l(0) eq 3) then cleanstack,im
