PRO gofindradiusandcenter,im_in,x0,y0,radius
 ...
 ;---------------------------------------------------------------------------------------
 ; Code to inspect output from the EFM method
 ; 2 patch version
 ; NOTE this version for synthetic images only
 ;---------------------------------------------------------------------------------------
 common vizualize,viz
 viz=0
 ;---------------------------------------------------------------------------------------
 for iflag=1,2,1 do begin	; set 1 or 2 for 4/5ths or 2/3rds of radius to place DS patch
     if (iflag eq 1) then idstring='45_efm'	
     if (iflag eq 2) then idstring='23_efm'
     ww=11
     nn=512
     im=fltarr(nn,nn)
     mask=fltarr(nn,nn)
     ...
     ; point to where the synthetic observed images are 
     ...
     for ifil=0,n-1,1 do begin
         ...
         fname=fitsname
         strput,fname,'d',13
         subdirname=strcompress(obspath+'FORCRAY/'+fname,/remove_all)
         ; Get the target - model
         openr,1,strcompress(subdirname+'/'+'output.raw',/remove_all)
         nn=512
         im=dblarr(nn,nn)
         readu,1,im
         close,1
         ...
         pctim=(im-ideal_im)/ideal_im*100.0
         writefits,strcompress('PCTerrorImage_'+fitsname,/remove_all),[[[pctim]],[[ideal_im]],[[observed]]]
