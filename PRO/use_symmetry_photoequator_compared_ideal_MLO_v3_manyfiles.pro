...
; Version 3.  Based on 'drizzling points'
; like V2 but does files froma  list and produces PostSCript for each
;---------------------------------------------------------------------------------
;
 !P.CHARSIZE=1.2
 !P.thick=3
 !x.thick=2
 !y.thick=2
 openw,103,'other_regression_idealisXobsisY.txt',/append
 openr,88,'allfilestoratio.txt'
 while not eof(88) do begin
 str=''
 readf,88,str
 print,str
 if (str eq 'stop') then stop
 bits=strsplit(str,'/',/extract)
 bits2=strsplit(bits(3),'.',/extract)
 nametouse=bits2(0)+'.'+bits2(1)
 cube=readfits(str,h)
 ...
 for eza_find=!pi/2.-eza_step,eza_step,-eza_step do begin
     ...
     printf,33,format=fmt1,eza(idx(jdx(0))),sza(idx(jdx(0))),coords(0),coords(1)
     endif
     endif
     endfor
 close,33
 ...
 nldx=n_elements(ldx)
 ndrizzle=1000
 openw,77,'other_symmetricstoplot.dat'
 for idrizzle=0,ndrizzle-1,1 do begin
 print,'Drizzle ',idrizzle,' of ',ndrizzle
 ...
 irow=coords(1)
     ...
     close,77
     ; plot it
     data=get_data('other_symmetricstoplot.dat')
...
device,xsize=18,ysize=24.5,yoffset=2
     device,filename=strcompress('ther_albedoratios_'+nametouse+'.ps',/remove_all)
     plot,title=nametouse+' Black: observed ratio, Red: ideal ratio',xstyle=3,ystyle=3,yrange=[min([obsratio,idealratio]),max([obsratio,idealratio])],mean_eza,obsratio,xtitle='EZA',ytitle='A/B and A*/B*',psym=7
...
;    oplot,mean_sza,yhat2,color=fsc_color('blue')
     ...
     dummy=linfit(idealratio,obsratio,/double,sigma=sigs)
     printf,103,format='(a,4(1x,f9.4))',nametouse,res(0),res(1),sigs(0),sigs(1)
     print,format='(a,4(1x,f9.4))',nametouse,res(0),res(1),sigs(0),sigs(1)
