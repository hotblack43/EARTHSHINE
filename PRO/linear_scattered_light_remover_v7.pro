...
;fitsname=strmid(str,xx,strlen(str)-xx)
 ...
 PRO go_clean_lunar_disc,res,theta,theta_step,clean_image,radii,angle,removed_light,whichcleaned,iflog
 ...
 PRO go_fit_line,filename,res,iflog
 ...
 y=reform(data(2,*))
	...
	if (iflog eq 1) then begin
     res=ladfit(x,alog10(y),/double) & yhat=res(0)+res(1)*x
     ;res=linfit(x,alog10(y),/double,yfit=yhat)
	endif
     if (viz eq 1) then begin
         window,1,xsize=400,ysize=300
         plot,x,y,psym=7,ystyle=1,title='Angle='+string(theta(0)),xtitle='Distance from Moon ctr.'
	 oplot,x,yhat,color=fsc-color('red'),thick=2
         endif
 return
 end
 ...
 ; Will work on log images given the iflog=1
 ;----------------------------------------------------
...
;............................
 ...
 for theta=0.0d0,180.0d0-theta_step,theta_step do begin
     ...
     print,'Theta=',theta
         idx=where(angle ge theta and angle lt theta+theta_step and radii gt radius*1.05)
     ...
     close,44
     go_fit_line,'bins.dat',res,iflog
     go_clean_lunar_disc,res,theta,theta_step,clean_image,radii,angle,removed_light,whichcleaned,iflog
     endfor	; end theta
...
;............................
 ...
 PRO get_observed_image,inname,observed_image,header,cg_x,cg_y
 common circleSTUFF,circle,radius,moon_coords
 common vizualise,viz
 ...
 ; Version 7: Like version 6 - but also outputs a result from using log01(image)
 ;.....................................................................................
 common paths,path
 common circleSTUFF,circle,radius,moon_coords
 t1=systime(/julian)
 ...
 common lineandpoint,line,point1
 common which,whichcleaned
 inpath='NOISEADDED_161/OUTPUT/'
 outpath0=inpath+'BBSO_CLEANED/'
 outpath1=inpath+'BBSO_CLEANED_LOG/'
 line=[256,0,256,511]
 point1=[511,0]
 files=file_search(inpath+'LunarImg_*.fit',count=n)
 print,'Found ',n,' files to work on.'
 for i=0,n-1,1 do begin
	for iflog=0,1,1 do begin
     gostripthename,files(i),fitsname
     observed_image_name=files(i)
     get_observed_image,observed_image_name,observed_image,header,cg_x,cg_y
