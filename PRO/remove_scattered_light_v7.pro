...
example_power=2.5	; for King profile
...
imin=readfits(path+'ANDREW/stacked_new_349_float.FIT')
l=size(imin,/dimensions)
writefits,strcompress(path+'Original_image_readin_'+exp_str+'_EX3.fit'),imin
return
end
...
if (strupcase(method_str) eq 'LINEAR') then example3,observed_image,inside,outside,imin2	; Ex. 3 just reads in a real image (to be used with linear method only)
;----------------------------------------------------------
