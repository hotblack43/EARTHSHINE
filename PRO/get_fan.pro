PRO use_cusp_angle_build_fan,im,cangle,x0,y0,radius,rad,line
 disp=im
...
;stop
 ...
 outrange=r_step*13
 w=40.	; degrees
     if (ipointer eq  1) then idx=where(theta ge cangle-w and theta lt cangle+w and xline gt x0)
     if (ipointer eq -1) then idx=where(theta ge cangle-w and theta lt cangle+w and xline lt x0)
     disp(idx)=max(im)
tvscl,disp
 ;
...
tvscl,disp
 ...
 im=readfits('data/pth/DARKCURRENTREDUCED/SELECTED_3/2456045.8565493MOON_B_AIR_DCR.fits',h)
 JD=2456045.8565493d0
 RADIUS  =         137.710 ;/Radius estimated from model image               
 X0      =         279.290 ;/Center is here in a model image                 
 Y0      =         260.490 ;/Center is here in a model image                 
 cangle=-cusp_angle(JD,'mlo')/!dtor
 ;..................
 use_cusp_angle_build_fan,im,cangle,x0,y0,radius,rad,line
 plot,rad,line
 end
