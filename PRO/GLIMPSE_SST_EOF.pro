...
print,'Done!'
help,lon,lat,SST
; Repack into land or sea only
for ilat=0,n_elements(lat)-1,1 do begin
factor=0.0001+sqrt(cos(abs(lat(ilat))/360.*2.*!pi))
for ilon=0,n_elements(lon)-1,1 do begin
if (lsm(ilon,ilat) eq 1) then SST(ilon,ilat,*)=randomn(seed,n_elements(time))
SST(ilon,ilat,*)=SST(ilon,ilat,*)*factor
endfor
endfor
; 
nlon=n_elements(lon)
