...
PRO eshine_16_particularsynthimage_nolroclem
...
;-----------------------------------------------------------------------
hapkeG=0.6
moon_albedo  = 0     ; 0=0.0720 (uniform) , 1=Clementine/HIRES 750 nm albedos
...
earth_albedo_uniform_value=0.31 ; the value of the albedo if earth_albedo_type=0
single_scattering=earth_albedo_uniform_value
;single_scattering = 0.303 ; used only if earth_albedo = 0
...
for iJD = JDstart,JDend,JDstep do begin
  ...
  eshine_core_20_nolroclem, iJD, phase_angle, lat_lib, lon_lib, PA_lib, $
...
; get_pixel_ratio,image_I,ratio
 ratio=-911
  ...
  out_ideal_name=strcompress('OUTPUT/IDEAL/SMOOTH/ideal_LunarImg_'+string(iframe,format='(I4.4)')+'.fit',/remove_all)
...
JDnumstr=string(JD,format="(f15.7)")
  out_ideal_name=strcompress('OUTPUT/IDEAL/SMOOTH/ideal_LunarImg_SCA_'+SCAnumstr+'_JD_'+JDnumstr+'.fit',/remove_all)
  writefits, out_ideal_name, image_I, header
	print,'image_I - MIN ne 0:',min(image_I(where(image_I ne 0)))
	print,'Wrote to OUTPUT/IDEAL/'
 get_lun,ijb & openw,ijb,'nameofparticularsynthimage.txt' & printf,ijb,out_ideal_name & close,ijb & free_lun,ijb
  ...
  writefits, strcompress('OUTPUT/SMOOTH/LunarImg_'+string(iframe,format='(I4.4)')+'.fit',/remove_all), image_I/max(image_I)*60000.0d0, header
  writefits, strcompress('OUTPUT/SMOOTH/SunMask_LunarImg_'+string(iframe,format='(I4.4)')+'.fit',/remove_all), mask 
