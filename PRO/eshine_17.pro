;===============================================================================
 ...
 ;-----------------------------------------------------------------------
 ; Set multi-reading options for dual corePCs
 ;-----------------------------------------------------------------------
...
; or manually assign the value
 JD = 2455641.6380796d0
 phase_angle = 0.0
 lat_lib = 0.0
 lon_lib = 0.0
 PA_lib  = 0.0
 ...
 if strcmp(obsname,'MEEQ  X,Y,Z',/fold_case) then begin
     ...
     obslon = 347.3
     obsalt = 15.0
     endif else if strcmp(obsname,'ROLO',/fold_case) then begin
     obssys = 'GEO'
     obslat = 32.2
     obslon = 111.5
     obsalt = 2148
 ...
 earth_albedo_type = 0     ; 0 is uniform (for value see below),  1=cloud-free Earth
 earth_albedo_uniform_value=0.303 ; the value of the albedo if earth_albedo_type=0
 ...
 CCDfactor = 1                   ; a divider to change both imsize and pixelscale
 imsize = fix(CCDsize/CCDfactor) ; image size in terms of pixels
 pixelscale = 	6.68*CCDfactor      ; arc seconds per pixel for the eShine telescope
 ...
 device_str = 'win'	; if running on IDL on a Windows PC
 device_str = 'ps'	; to generate PostSCript on either platform
 device_str = 'X'	; if running under *nix
 ...
 for iJD = JDstart,JDend,JDstep do begin
     ...
     moon_albedo, moon_BRDF, earth_albedo_type, earth_albedo_uniform_value, $
     ...
     ratio=-911
     get_pixel_ratio,image_I,ratio,CCDFactor
     ...
     if (if_sky eq 1) then add_telescope_and_filter_effects, image_16bit, filter, image_info.phase_angle_E, imsize, $
     filterfact, skylevel
     ...
     sxaddpar, header, 'JULIAN',string(double(IJD),format='(d20.8)'), ' simulation JULIAN DAY'
     sxaddpar, header, 'MPHASE',string(double(phase_angle_M),format='(d20.8)'), 'Lunar phase angle (S-M-E)'
 ...
 close,83
     ...
     print,'OUTPUT: look for JPEG image in OUTPUT/JPEGS/ '
     print,'        look for ideal image in OUTPUT/IDEAL/ '
     print,'        look for information in OUTPUT/INFO/ '
     print,'        look for realistic image in OUTPUT/'
