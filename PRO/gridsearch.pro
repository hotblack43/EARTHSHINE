...
PRO determineoffsetfactor,im,blurred,factor,offset
 ...
 fitsky=x0-radius-20              ; middle of sky force-fit patch in x 
                                  ; set from circle center minus an offset
 colwhere=coo(1)
...
;	print,'factor, offset = ',factor,offset
 ...
 ; grid-searched set of PSF.
 ;---------------------------------------------------------------------------
 ; The error to minimize is evaluated on the sky between certain columns.
...
;openw,34,'JD.dat' & printf,34,format='(f20.7)',dectime& close,34
 ...
 b=440
;..
 a=0	; a and b are left and right column limits for error evaluation
 b=250	
;..
