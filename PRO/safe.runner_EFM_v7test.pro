PRO gofitSECONDway,a,alfa1,alfa2,kinkpoint,l,results,basicfilename,outpath,header,cutoffval
 common ims,observed,source,residual,mask,trialim,cleanup
     ...
     fixed:0, limited:[0,0], limits:[0.D,0],step:0.1}, 4)
     parinfo[0].fixed = 0
     parinfo[1].fixed = 0
     parinfo[2].fixed = 0
     parinfo[3].fixed = 0
...
device,xsize=18,ysize=24.5,yoffset=2
 ...
 sxaddpar, header, 'ALFA1', alfa1, 'PSF alfa1 found via EFM.'
 sxaddpar, header, 'ALFA2', alfa2, 'PSF alfa2 found via EFM.'
...
return
 ...
 ;--------------------------------------------------------------------
 ; RUNNER version 5. EFM method.
 ; Unlike version 4, version 5 will read DISCX0 and DISCY0 and RADIUS from header
 ;--------------------------------------------------------------------
 !P.CHARSIZE=2
 common ims,observed,source,residual,mask,trialim,cleanup
 common errs,errorwholeimage,errorinabox,idealerrorinabox,b,idealbiasinbox
...
; path to input files
 str=strcompress(lowpath+'DARKCURRENTREDUCED/SELECTED_1/2456045.8248641MOON_V_AIR*.fits',/remove_all)
