...
;--------------------------------------------------------------
; Code to inspect images and to move the bad ones manually
;--------------------------------------------------------------
;
path='/data/pth/DATA/ANDOR/MOONDROPBOX/JD2455847/'
files=file_search(path,'*MOON*.fits',count=n)
a=''
ic=0
while (a ne 'q' and ic lt n) do begin
im=readfits(files(ic),h,/silent)
mn=max(im)
print,mn,files(ic)
if (mn lt 10000 or mn gt 56000) then begin
	file_move,files(ic),strcompress(path+'BAD/',/remove_all)
	print,'I moved the file ',files(ic),' to the BAD directory'
endif else begin
	tvscl,(im)
	;print,'If OK then press space bar. "q" to quit'
	;a=get_kbrd()
endelse
	ic=ic+1
endwhile
