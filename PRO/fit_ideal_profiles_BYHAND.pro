...
; actioninidcator=1 implies the image under treatment is the observed image, not the model.
     ...
     if (kdx(0) ne -1) then print,'1 ',-mean(radii(idx(kdx))),median(pixels),stddev(pixels)
     endfor
 markedupimage=im
...
if (w3 ne w4) then begin
 for r=radius+w3,radius+w4,r_step do begin
     ...
     if (nfan gt 30) then print,'2 ',mean(radii(jdx(kdx))),median(pixels),stddev(pixels)
     endfor
 endif
 close,66
...
; build a new header based on the old one along with
 ...
 common stuff117,name,use1,use2,im1,im2,raw,RMSE,x0,y0,radius,nbands,w1,w2,w3,w4
 radius=139.5d0
print,'FIND THE FUDGE FIND THE FUDGE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
 common namingstuff,JD,labelstr,markedupimage,if_smoo,if_jiggle,jiggle_ampl,paramname
 common Y, Y
 common fanstuff,rad_raw,line_raw,rad_folded,line_folded
 common thetaflags,iflag_theta,radii,theta,xline,yline
...
;writefits,'im2_justaftercreation.fits',im2
 ...
 ;..............................................................................
 ; Version 30 of code that finds albedo by model-fitting
 ; Special code set up to enable later fitting of ideal images.
 ; V 30 outputs convolved ideal images with an 'observed' header
 ; to enable this later step.
 ;..............................................................................
 common stuff117,name,use1,use2,im1,im2,raw,RMSE,x0,y0,radius,nbands,w1,w2,w3,w4
 common namingstuff,JD,labelstr,markedupimage,if_smoo,if_jiggle,jiggle_ampl,paramname
 common fanstuff,rad_raw,line_raw,rad_folded,line_folded
...
close,91
         ...
         if (x0-radius gt 31 and rawncols-1-(x0+radius) gt 31 ) then begin
             ...
             im2=im2_good
             ;..........................................
help,albedo,erralbedo,alfa1,bwpr,pedestal,xshift,yshift,acoeff,lamda0
             ...
             openw,63,'LRO.bootstrapperfitting_PSFisalfa1only.txt',/append
             flux=totfl/exptime
             printf,63,format=fmtstr44,JD,albedo,erralbedo,alfa1,bwpr,pedestal,xshift,yshift,acoeff,lamda0,RMSE,totfl,zodi,SLcounts,flux,name,labelstr,filter,parinfo.fixed
             close,63
