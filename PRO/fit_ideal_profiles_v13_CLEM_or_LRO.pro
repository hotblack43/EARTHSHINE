...
PRO getcontrastfromfiltername,filter,contrast
contrast=-1e22
if (filter eq 'B') then contrast = 0.85
if (filter eq 'V') then contrast = 0.92
if (filter eq 'VE1') then contrast = 0.86
if (filter eq 'VE2') then contrast = 0.58
if (filter eq 'IRCUT') then contrast = 0.89
return
end
...
; BS fan
 ...
 ; recall that im1 has albedo=0 so there is nothing to scale??
 im2contrasted=im2
; pick out the fainter parts of the image, but not the sky - ie just the DS
 idx=where(im2 lt max(im2)/500. and im2 ne 0.0)
; get the mean of the DS
 mv=mean(im2(idx),/double)
; subtract the mean of that part, scale the rest, add the mean back in
 im2contrasted(idx)=(im2(idx)-mv)*contrast+mv
; the result is that the DS has been contrast scaled
 return
 end
...
writefits,'im2replaced.fits',im2
 ...
 corefactor=1.000
 contrast=1.0
;getcontrastfromfiltername,filter,contrast
 yshift=0.0;47131281
 zodi=0.0
 SLcounts=0.0
 p = [alfa1,rlimit,pedestal,albedo,xshift,corefactor,contrast,yshift,zodi,SLcounts]
 if (file_exist('lastfit_v11') eq 1) then begin
     get_lun,xyz
     pp=p
     openr,xyz,'lastfit_v11'
     readf,xyz,pp
     close,xyz
     free_lun,xyz
     p=pp
...
;p(6)=1.0	; B contrast
 ...
 contrast=parms(6)
 yshift=parms(7)
 get_lun,xyz & openw,xyz,'lastfit_v11' &printf,xyz,format='(10(1x,f15.9))',parms & close,xyz & free_lun,xyz
 print,'Solution:'
...
print,'     alfa   rlimit  pedestal  albedo   xshift   corefac contrast  yshifta     ZL       SL'
 ...
 free_lun,hjkl
 ;...get the image
 spawn,'idl go_get_particular_synthimage_217.pro'
...
close,91
         ...
         if (x0-radius gt 31 and rawncols-1-(x0+radius) gt 31 ) then begin
             ...
             openw,63,'tescases_NOV25_2015.txt',/append
             flux=totfl/exptime
             printf,63,format=fmtstr44,JD,albedo,erralbedo,alfa1,rlimit,pedestal,xshift,yshift,corefactor,contrast,RMSE,totfl,zodi,SLcounts,flux,name,labelstr
             close,63
