...
; set up the ideal image wighted by albedo
 ...
 ;----------------------------------------------------
 ; version 4b of Full Forward Method
 ; Like version 4 but uses the syntheticoon that takes SEED as ana rgument
 ; and also loops so that bootstrappig is possible
 ;----------------------------------------------------
 !P.CHARSIZE=2
 common ims,observed,trialim,idealused,ideal,blackimage,whiteimage
 common errs,functionvalue,residual2BOX
 ...
 factor=3.78
 pedestal=400.0
 nloop=100	; How many Monte Carlo trials tomake
 nimages=100	; number of noisy images to coadd
 alfawanted=1.6	& alphastr='1p6'
 openw,63,'iloop_collection.txt'
 for iloop=1,nloop,1 do begin
 print,'THIS IS LOOP # ',iloop
 ; generate a synthetic observed image
 spawn,'cp veryspecial_moon_0p297.fit ideal_in.fits'
 ideal=readfits('ideal_in.fits',header)
 ...
 spawnstr='./syntheticmoon usethisidealimage.fits '+strcompress('synth_observed_'+alphastr+'.fits ')+string(alfawanted)+string(nimages)+' '+string(seed)
 spawn,spawnstr
...
gogetjulianday,header,jd
 fmt='(f20.7,3(1x,f14.10),1x,g12.1,2(1x,f12.6),1x,i4)'
 print,format=fmt,jd,alfa,a,albedo,999.999,functionvalue,residual2BOX,iloop
 printf,63,format=fmt,jd,alfa,a,albedo,999.999,functionvalue,residual2BOX,iloop
