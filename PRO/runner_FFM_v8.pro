...
;radius=median(reform(data(2,*)))
 ...
 im(x0:x0,*)=max(im)
 im(*,y0:y0)=max(im)
 if (ifviz eq 1) then tvscl,hist_equal(im)
 return
 end
...
residual(idx)=0.0
 ...
 ; plot
 if (ifviz eq 1) then plot3things,observed,trialim,residual
 return, residual
 END
 
 ;----------------------------------------------------
 ; runner version 8 of Full Forward Method
 ; is called from runner.scr
 ;----------------------------------------------------
 !P.CHARSIZE=2
 common ims,observed,trialim,idealused,ideal,blackimage,whiteimage,cleanup
 common errs,functionvalue,residual2BOX
...
;    parinfo[3].limits(1)  = 1.0
 ...
 fmt='(f20.7,2(1x,f14.10),8(1x,f15.5),4(1x,f8.3),1x,a)'
