...
files=file_search('/data/pth/DATA/ANDOR/MOONDROPBOX/JD2455849/*.fits',count=n)
oldJD=1.0
...
get_info_from_header,h,'EXPOSURE',req_exptime
get_info_from_header,h,'ACT',cycle_time
flx=mn/measured_exptime
delta=(jd-oldJD)*24.*3600.-2.*cycle_time
fmtstr='(i3,1x,f15.6,7(1x,g10.5),1x,a,1x,a)'
print,format=fmtstr,ifile,jd,measured_exptime,req_exptime,cycle_time,temp,mn,flx,delta,filter,files(ifile)
printf,55,format=fmtstr,ifile,jd,measured_exptime,req_exptime,cycle_time,temp,mn,flx,delta,filter,files(ifile)
oldJD=jd
endfor
close,55
