...
openr,1,'ALTAIR.list1'
path='/media/SAMSUNG/SCIENCEPROJECTS/MOONDROPBOX/JD2455834/'
ic=0
...
if (max(im) gt 20000 and exp_time1 eq exp_time3 and (exp_time1 eq exp_time2) ) then begin
dark=(dark1+dark2)/2.0
im=im-dark
print,ic,exP_time2,' ',max(im),mean(dark),' ',b
...
for i=0,n_elements(counts)-1,1 do begin
	writefits,strcompress('ALTAIR_coadded_'+string(fix(exptimes(i)))+'_seconds.fits',/remove_all),reform(summdimages(*,*,i)/float(counts(i)))
endfor
end
