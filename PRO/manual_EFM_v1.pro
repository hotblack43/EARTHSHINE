...
trialim=a+b*source_folded
print,'Tot obs: ',total(observed*mask)
print,'Tot mod: ',total(trialim*mask)
print,'Difference: ',total(observed*mask)-total(trialim*mask)
;.............................................................
; get residuals wrt observed image
 cleanup=observed-trialim
!P.MULTI=[0,1,3]
plot_io,observed(*,y0),xstyle=3
oplot,trialim(*,y0),color=fsc_color('red')
plot,observed(*,y0),xstyle=3,yrange=[-10,30]
oplot,trialim(*,y0),color=fsc_color('red')
w=3
plot,avg((observed(*,y0-w:y0+w)-trialim(*,y0-w:y0+w))/observed(*,y0-w:y0+w),1),xstyle=3,yrange=[-0.1,0.1]
oplot,trialim(*,y0),color=fsc_color('red')
print, "Press 'u' or 'd' to adjust alfa"
da=0.0
ans=get_kbrd()
        if (ans eq 'u') then begin
        da=0.01
        endif
...
return
 ...
 ;--------------------------------------------------------------------
 ; manual_EFM RUNNER version 1. EFM method.
; where anchoring to DS sky and flux conservation is imposed. Alfa is fitted by hand
 ;--------------------------------------------------------------------
 !P.CHARSIZE=2
 common ims,observed,source,residual,mask,trialim,cleanup
 common errs,errorwholeimage,errorinabox,idealerrorinabox,b,idealbiasinbox
...
; path to input files
 str=strcompress(lowpath+'DARKCURRENTREDUCED/SELECTED_1/2455943.0*.fits',/remove_all)
