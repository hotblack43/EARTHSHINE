...
FUNCTION beta,S,T,P
    ...
    ;Input:
     ...
     ;   T = Temperature,  [°C]
i=0
     for Tin=10.,0.,-1. do begin
     ;   P = Pressure,     [dbar]
     length=0.0
     dstep=10.	; meters
     openw,33,'plotme.dat'
     for depth=0,7000.,dstep do begin
     pin=depth	; Pressure in dbar
     p=pin
     s=sin
     t=tin
alfa=alpha(S,T,P)

print,s,t,depth,pin,alfa,length
     printf,33,depth,length
     length=length+(1.0+alfa)*dstep
     endfor
     close,33
     data=get_data('plotme.dat')
     x=reform(data(0,*))
     y=reform(data(1,*))
     if (i eq 1) then plot,x,y-x,ystyle=1,xtitle='Depth (meters)',ytitle='Accumulated expansion at !7D!3T= 1 C everywhere',charsize=1.2,title='Case: uniform temperature. Temperature = 1 C, increase = 1 C'
if (i gt 1) then oplot,x,y-x
i=i+1
endfor
     end