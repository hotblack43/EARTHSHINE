...
openw,88,'mean_sw_allsky.dat'

 ncfile = 'CERES_SYN1deg-Day-lite_Terra_Ed2.6_Subset_200003-200303.nc'
...
; ----------- the following is a data read/write example---------------
 ...
 printf,ounit,'Month Day Year     time      lat      lon       Value'  
  ...
  ENDFOR ;ilon
  ENDFOR ;ilat
     print,date(itime),mean(data(*,*,itime))
	arr=data(*,*,itime)
     printf,88,date(itime),mean(arr(where(arr gt 0)))
  ENDFOR ;itime
