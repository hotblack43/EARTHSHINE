...
if n_params() ne 3  then begin
	...
	if (sz[1] ne sr[1]) or (sz[2] ne sr[2]) then $
		...
		; sense if they don't. There may be some circumstances where you'd want
...
wg = where(goodlist ne 0, ng)
	...
	;if sz[1] ne sr[1] or sz[2] ne sr[1] then $
		;message,"reference image must have same dimensions as images to register!"
	if sr[0] gt 2 then $
...
;if max(abs((imgs0[*,*,0] - ref0)/ref0),/nan) lt 0.01 then startreg = 1 else startreg=0
;startreg=0
...
;-----------------------------------------------------------------
	...
	;	if keyword_set(display )then begin wset,0 & shade_surf,cor[0>x-s:x+s<(sz[1]-1),0>y-s:y+s<(sz[2]-1)],$
	;			title="Fourier Cross-correlation" & endif
	;	if keyword_set(display )then begin wset,1 &  shade_surf,g,title="fit gaussian" & endif
		...
		; the shift is going to be the difference between the image center, nx/2
		; and the gaussian peak location (plus fitting box offset),  (0>(x-s))+a[4]
		;
		; rounding code added 2006-05-25 to deal with images with odd-sized axes
		; Oddly, IDL does NOT return a convolution which is centered on a
...
;-----------------------------------------------------------------
	if strc(method) eq "FHP" then begin ; old-style fourier cross-correlation, fit by Gaussian
		...
		if ~(keyword_set(ftpsf)) then begin
			; we can effect the high pass filtering in Fourier space just by
...
;-----------------------------------------------------------------
	...
	;	if keyword_set(display )then begin wset,1 &  shade_surf,g,title="fit gaussian" & endif
		...
		; changed to 40 on 2006-06-09; problems with offset drift during LGS
		; mode.
...
;-----------------------------------------------------------------
	if strc(method) eq "C" then begin ; StarFinder method
		...
		;if not(keyword_set(silent)) then print,["dxc= ","dyc= "]+strc(dc)
		shifts[*,n2]=dc
...
;-----------------------------------------------------------------
	if strc(method) eq "G" then begin ; gaussian fitting method
	    ...
	    repeat begin
	        ...
	        ;subreg,im1,im2s,shifts,/silent,method=method,boxsize=boxsize; calculate the new relative shifts
	        res2 = mpfit2dpeak(im2s,g2)
	        s = [g1[4]-g2[4],g1[5]-g2[5]]
	        dxy=dxy+s          ; new overall shifts for next round
	        error=sqrt(total(s^2))
	        c_iter=c_iter+1
	        ;print,"Iter "+strc(c_iter)+", shifts="+strc(s[0])+", "+strc(s[1])+"      Error: "+strc(error)
...
;-----------------------------------------------------------------
	if strc(method) eq "O" then begin ; goddard method
		...
		cntrd,imagei,x2,y2,xc2,yc2,5
	 	;if not(keyword_set(silent)) then	print,["dxn= ","dyn= "]+strc([xc1,yc1]-[xc2,yc2])
