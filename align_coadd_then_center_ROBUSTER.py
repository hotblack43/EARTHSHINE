...
def ransac_then_fit_circle(
    ...
    """
    Candidate edge points from gradients -> RANSAC circle -> inlier set -> robust centre + robust (quantile) radius.
    """
    ...
    # Robustify: keep a good centre, then compute radius from a high quantile of inlier distances.
    # This helps when the limb points form only an arc and/or are biased inward by PSF/thresholding.
    cx_ls, cy_ls, r_ls = fit_circle_least_squares(px[mask], py[mask])

    d = np.sqrt((px[mask] - cx_ls) ** 2 + (py[mask] - cy_ls) ** 2)

    use_q = float(radius_quantile)
    if not np.isfinite(use_q) or use_q <= 0.0:
        # Fall back to least-squares radius
        cx, cy, r = float(cx_ls), float(cy_ls), float(r_ls)
        method = "LS"
    else:
        # Clamp quantile to a sensible range
        if use_q < 0.50:
            use_q = 0.50
        if use_q > 0.999:
            use_q = 0.999

        r_q = float(np.quantile(d[np.isfinite(d)], use_q)) if np.isfinite(d).any() else float("nan")
        if not np.isfinite(r_q):
            cx, cy, r = float(cx_ls), float(cy_ls), float(r_ls)
            method = "LS (quantile failed)"
        else:
            cx, cy, r = float(cx_ls), float(cy_ls), float(r_q)
            method = f"Q{use_q:.3f}"

    if not (r_min <= r <= r_max):
        raise RuntimeError(f"Circle radius out of bounds: r={r:.2f} (method={method})")

    if verbose:
        print(
            f"  COADD circle (RANSAC->centre + {method} radius): "
            f"cx={cx:.2f} cy={cy:.2f} r={r:.2f} "
            f"(r_ls={float(r_ls):.2f}) inliers={nin}"
        )
...
def main() -> int:
    ...
    # Robust disc radius estimation (helps when limb points form only an arc and/or are biased inward).
    # Use 0 to disable and fall back to least-squares radius.
    ap.add_argument(
        "--radius-quantile",
        type=float,
        default=0.95,
        help=(
            "After RANSAC, estimate radius as this quantile of inlier distances to the fitted centre. "
            "Use 0 to disable and fall back to least-squares radius. Default: 0.95"
        ),
    )
