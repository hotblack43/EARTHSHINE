PRO find_model_radius,im,radius,CCDsize
row=total(im,1)
help,row
for i=0,CCDsize-2,1 do begin
;for i=0,510,1 do begin
;if (row(i) eq 0 and row(i+1) gt 0.0) then guess1=256-i
if (row(i) eq 0 and row(i+1) gt 0.0) then guess1=CCDsize/2-i
endfor
for i=CCDsize-2,1,-1 do begin
;for i=511,1,-1 do begin
if (row(i) eq 0 and row(i-1) gt 0.0) then guess2=i-CCDsize/2
...
PRO eshine_16_particularsynthimage_assigningIMAGEparametersinfiles
...
filnavn=get_data('JDtouseforSYNTH')
JD=filnavn(0)
;lat_lib = 0.0
;lon_lib = 0.0
;PA_lib  = 0.0
...
single_scattering=filnavn(0)
earth_albedo_uniform_value=single_scattering ; the value of the albedo if earth_albedo_type=0 (obsolete comment?)
...
CCDsize = get_data('CCDsize.dat')
CCDsize = fix(CCDsize(0))
help,CCDsize
...
pixelscale = 6.67*CCDfactor/(CCDsize/512)      ; arc seconds per pixel. Our telescope has 6.7"/pixel
...
; get_pixel_ratio,image_I,ratio
 ratio=-911
  ...
  sxaddpar, header, 'ALBEDO',string(double(single_scattering),format='(d20.8)'), ' Earth single-scat albedo'
  ...
  find_model_radius,image_I,radius,CCDsize
  sxaddpar, header, 'RADIUS',radius, 'Radius estimated from model image'
; writefits, out_ideal_name, image_I, header
;----------------
;str = string(float(fix(10000*single_scattering))/10000)
;help,str
;SCAnumstr = strmid(strtrim(str.replace('.','p')),0,11)
;help,SCAnumstr
;----------------
...
str = string(scaNUM)
help,str
SCAnumstr = str.replace('.','p')
help,SCAnumstr
stop
;----------------
JDnumstr=string(JD,format="(f15.7)")
mphase,JD,illfrac
illfrac_str=string(illfrac,format="(f6.4)")
  out_ideal_name=strcompress('OUTPUT/IDEAL/ideal_LunarImg_SCA_'+SCAnumstr+'_JD_'+JDnumstr+'_illfrac_'+illfrac_str+'.fit',/remove_all)
  writefits, out_ideal_name, image_I, header
	print,'image_I - MIN ne 0:',min(image_I(where(image_I ne 0)))
	print,'Wrote to OUTPUT/IDEAL/'
	;print,'Wrote to /data/users/pth/OUTPUT/IDEAL/'
get_lun,ijb 
openw,ijb,'nameofparticularsynthimage.txt'
printf,ijb,out_ideal_name
close,ijb
free_lun,ijb
...
; writefits, strcompress('OUTPUT/LunarImg_'+string(iframe,format='(I4.4)')+'.fit',/remove_all), image_I/max(image_I)*60000.0d0, header
  writefits, strcompress('OUTPUT/SunMask_JD_'+string(JD,format='(f15.7)')+'.fit',/remove_all), mask 
	print,'image_16bit - MIN ne 0:',min(image_16bit(where(image_16bit ne 0)))
	print,'Wrote to OUTPUT/'

; The image_info is now handled in eshine_core_ where a FITS header is written into the lonlatSEL*** images.
